...
export class FiscalDocument {
  ...
  /**
   * Rehydrates and validates a complete fiscal-document aggregate snapshot.
   *
   * @param state - Commercial, tax, payment, lifecycle, and issuance state to preserve.
   * @returns A validated immutable fiscal-document aggregate.
   * @throws {@link FiscalFailure} When any invariant or monetary reconciliation fails.
   */
  ...
  /**
   * Records the evidence produced when an outbound draft is legally issued.
   *
   * @param input - Assigned number, effective timestamps, and provider-neutral evidence.
   * @returns A new issued aggregate; the current draft remains unchanged.
   * @throws {@link FiscalFailure} When the document is not an outbound draft or issuance data is invalid.
   */
  ...
  /**
   * Records a supplier-issued document as received without claiming Kontave issued it.
   *
   * @param input - Supplier number, effective timestamps, and supporting evidence.
   * @returns A new received aggregate; the current draft remains unchanged.
   * @throws {@link FiscalFailure} When the document is not an inbound draft or receipt data is invalid.
   */
