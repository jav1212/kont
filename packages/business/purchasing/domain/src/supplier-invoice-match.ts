...
export class SupplierInvoiceMatch {
   ...
   * @throws {PurchasingFailure} When allocations, currency, or lifecycle are invalid.
   */
  constructor(state: SupplierInvoiceMatchState) {
    if (!Number.isSafeInteger(state.version) || state.version < 0 || state.allocations.length === 0) {
      throw new PurchasingFailure(
        ...
        "Supplier invoice match state is invalid.",
      );
    }
    const allocations = state.allocations.map((allocation) => {
      if (allocation.orderLineId === null && allocation.receiptLineId === null) {
        throw new PurchasingFailure(
          "PURCHASE_INVOICE_MATCH_INVALID",
          "Invoice allocation must reference an order or receipt line.",
        );
      }
      if (
    ...
    if ((state.status === "draft") !== (state.confirmedAt === null)) {
      throw new PurchasingFailure(
        "PURCHASE_INVOICE_MATCH_INVALID",
        "Supplier invoice match lifecycle is inconsistent.",
      );
    }
    this.id = state.id;
   ...
   * @throws {PurchasingFailure} When already confirmed or the instant is invalid.
   */
