...
/**
 * Validates and freezes the legal/audit metadata of a Cestaticket amount rule.
 * @param input - Versioned amount rule and supporting sources.
 * @returns An immutable normalized rule copy.
 * @throws {VenezuelanPayrollFailure} When dates, sources, currency, or authority metadata are invalid.
 */
...
/**
 * Validates whether a Cestaticket delivery arrangement satisfies modality-specific evidence rules.
 * @param input - Delivery arrangement and employer workforce size.
 * @returns Nothing when the arrangement is legally representable.
 * @throws {VenezuelanPayrollFailure} When cash exceptions or supporting evidence are insufficient.
 */
...
/**
 * Assesses and reconciles the monthly Cestaticket benefit at the effective payment-date rate.
 * @param input - Rule, month, attendance, delivery, exchange snapshot, prior delivery, and settlement currency.
 * @returns A fully auditable monthly Cestaticket assessment.
 * @throws {VenezuelanPayrollFailure} When eligibility, rate, dates, amounts, or delivery evidence are invalid.
 */
