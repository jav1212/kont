...
export class PayrollRun {
  ...
  /**
   * Creates a new draft payroll run with zero totals and no calculated results.
   * @param input - Stable identity, definition, period, type, and settlement currency.
   * @returns A new immutable draft aggregate.
   */
  ...
  /**
   * Applies relationship calculation results and recomputes run totals atomically.
   * @param results - Non-empty relationship results in the settlement currency.
   * @param calculatedAt - Auditable calculation timestamp.
   * @returns A new calculated aggregate, with warning status when applicable.
   * @throws {PayrollFailure} When the transition, result set, or currencies are invalid.
   */
  ...
  /**
   * Approves a calculated run that contains no error messages.
   * @param approvedAt - Auditable approval timestamp.
   * @returns A new approved aggregate.
   * @throws {PayrollFailure} When the status is invalid or calculation errors remain.
   */
  ...
  /**
   * Confirms an immutable payroll result according to its approval policy.
   * @param confirmedAt - Auditable confirmation timestamp.
   * @param approvalRequired - Whether prior approval is mandatory.
   * @returns A new confirmed aggregate.
   * @throws {PayrollFailure} When the transition is invalid or calculation errors remain.
   */
  ...
  /**
   * Reverses a confirmed run while preserving the original calculated result.
   * @param reversedAt - Auditable reversal timestamp.
   * @param reason - Non-empty business reason for the reversal.
   * @returns A new reversed aggregate.
   * @throws {PayrollFailure} When the run is not confirmed or the reason is empty.
   */
