...
/**
 * Calculates one worker relationship deterministically from effective definitions and entries.
 * Expected evaluator failures are captured as result messages so the run retains a complete audit trail.
 * @param input - Worker snapshot, period, currency, definitions, entries, and policy references.
 * @returns The calculated relationship result, including balances, messages, and traces.
 * @throws {PayrollFailure} When an entry belongs to another relationship or the definition graph is invalid.
 */
...
/**
 * Topologically orders element definitions while preserving phase and priority semantics.
 * @param definitions - Effective payroll element definitions.
 * @returns A deterministic dependency-safe ordering.
 * @throws {PayrollFailure} When a dependency is missing or the graph contains a cycle.
 */
