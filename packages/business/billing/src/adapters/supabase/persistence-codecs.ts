import { z } from "zod";
import { BillingCycle, Currency, InvoiceStatus, ManualPaymentMethod, ManualPaymentStatus, PaymentMethodKind, SubscriptionStatus } from "../../domain";
...
export const billingPlanRowSchema = z.object({ id:z.uuid(),name:z.string(),max_companies:z.number().int().nullable(),max_employees_per_company:z.number().int().nullable(),price_monthly_usd:z.union([z.string(),z.number()]),price_quarterly_usd:z.union([z.string(),z.number()]),price_annual_usd:z.union([z.string(),z.number()]),is_contact_only:z.boolean(),included_modules:z.array(z.string()).catch([]),commercial_code:z.string().nullable().catch(null),products:z.object({slug:z.string()}).nullable().optional() });
