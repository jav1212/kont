...
import { BillingCycle, BillingFailure, Currency, ManualPaymentMethod, ManualPaymentStatus, money, type OrganizationEntitlements } from "../../src/domain";
import { organizationId, userId } from "@kontave/organizations/domain";
import { SubmitManualPaymentRequest, type BillingAuthorizationContext, type OrganizationBillingAuthorization, type OrganizationBillingRepository } from "../../src/application";
...
test("credit is internal and cannot be selected by a client application",async()=>{await assert.rejects(()=>new SubmitManualPaymentRequest(new Repository(),new Authorization()).execute(actor,organization,{planId:"plan-1",billingCycle:BillingCycle.Monthly,paymentMethod:ManualPaymentMethod.Credit},context),(cause:unknown)=>cause instanceof BillingFailure&&cause.code==="BILLING_PAYMENT_REQUEST_INVALID");});
