...
export class GoodsDispatch {
  ...
  /**
   * Rehydrates a goods dispatch while enforcing lifecycle and line invariants.
   * @param state Persisted dispatch state.
  ...
  /**
   * Confirms this draft dispatch.
   * @param value Confirmation timestamp.
   * @returns The next aggregate version and its stock event.
   * @throws {SalesFailure} When the dispatch is not a draft or the instant is invalid.
   */
  confirm(value: string): { readonly dispatch: GoodsDispatch; readonly event: SalesDispatchConfirmed } {
    ...
    return {
      dispatch,
      event: {
        type: "sales.dispatch_confirmed",
        eventId: operationKey,
        operationKey,
        dispatchId: this.id,
        companyId: this.companyId,
        customerId: this.customerId,
        effectiveDate: this.dispatchDate,
        occurredAt,
        lines: this.lines,
      },
    };
  ...
  /**
   * Reverses this confirmed dispatch.
   * @param value Reversal timestamp.
   * @returns The next aggregate version and its reversal event.
   * @throws {SalesFailure} When the dispatch is not confirmed or the instant is invalid.
   */
  reverse(value: string): { readonly dispatch: GoodsDispatch; readonly event: SalesDispatchReversed } {
    ...
    return {
      dispatch,
      event: {
        type: "sales.dispatch_reversed",
        eventId: `sales-dispatch-reversal:${this.id}:v${version}`,
