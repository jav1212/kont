...
export interface EmployeeRepository {
  list(companyId:CompanyId):Promise<readonly Employee[]>;
...
export class ListCompanyEmployees { constructor(private readonly repository:EmployeeRepository){}async execute(companyId:CompanyId){const rows=await this.repository.list(companyId);for(const employee of rows)employee.assertBelongsTo(companyId);return rows;} }
