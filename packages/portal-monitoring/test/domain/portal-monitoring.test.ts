...
import { PortalAvailability, summarizePortalMonitoring, type PortalStatus } from "../../src/domain";
...
test("uses the worst known portal state for the monitoring indicator", () => {
  assert.equal(summarizePortalMonitoring([portal("a", PortalAvailability.Operational), portal("b", PortalAvailability.Degraded)]).status, PortalAvailability.Degraded);
  assert.equal(summarizePortalMonitoring([portal("a", PortalAvailability.Degraded), portal("b", PortalAvailability.Down)]).status, PortalAvailability.Down);
});

test("does not report operational while an active portal has no evidence", () => {
  const snapshot = summarizePortalMonitoring([
...
test("reports the newest observation without inventing one for an empty portfolio", () => {
  assert.equal(summarizePortalMonitoring([]).observedAt, null);
  assert.equal(summarizePortalMonitoring([
