...
export interface OrganizationMembersRepository {
  list(organizationId:OrganizationId):Promise<readonly import("../domain").OrganizationMemberProjection[]>;
  invite(input:{organizationId:OrganizationId;actorUserId:UserId;email:string;roleId:string;rawToken:string;tokenHash:string;idempotencyKey:string;expiresAt:string}):Promise<{readonly member:import("../domain").OrganizationMemberProjection;readonly created:boolean}>;
  resend(input:{organizationId:OrganizationId;actorUserId:UserId;invitationId:string;rawToken:string;tokenHash:string;expiresAt:string;expectedVersion:number}):Promise<import("../domain").OrganizationMemberProjection>;
  ...
  update(input:{organizationId:OrganizationId;actorUserId:UserId;membershipId:string;roleId?:string;status?:"active"|"suspended";expectedVersion:number}):Promise<import("../domain").OrganizationMemberProjection>;
