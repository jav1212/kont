...
import { AuthenticationFailure, type AuthenticatedSession, type RefreshedAuthenticatedSession } from "../../src/domain";
import {
...
test("renews and retries an operation once with the updated access token", async () => {
  ...
  const coordinator = new SessionRefreshCoordinator(gateway);
  const tokens: string[] = [];
