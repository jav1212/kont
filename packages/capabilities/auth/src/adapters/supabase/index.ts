...
import { AuthenticationFailure, authenticatedSessionId, type AuthenticatedIdentity, type AuthenticatedSession, type SessionClientKind, type RefreshedAuthenticatedSession } from "../../domain";
...
export function createSupabaseAuthenticatedSessionRegistry(configuration:{readonly url:string;readonly serviceRoleKey:string}):AuthenticatedSessionRegistry{return new SupabaseAuthenticatedSessionRegistry(createClient(configuration.url,configuration.serviceRoleKey,{auth:{persistSession:false,autoRefreshToken:false,detectSessionInUrl:false}}))}
...
class SupabaseAuthenticatedSessionRegistry implements AuthenticatedSessionRegistry{
 ...
 async revoke(input:Parameters<AuthenticatedSessionRegistry["revoke"]>[0]){const{error}=await this.client.rpc("revoke_native_device_session",{p_user_id:input.userId,p_session_id:input.sessionId});if(error)throw sessionFailure(error)}
 async revokeOthers(input:Parameters<AuthenticatedSessionRegistry["revokeOthers"]>[0]){const{error}=await this.client.rpc("revoke_other_native_device_sessions",{p_user_id:input.userId,p_current_session_id:input.currentSessionId});if(error)throw sessionFailure(error)}
}
function nullable(value:unknown){return value===null||value===undefined?null:String(value)}
function readClient(value:unknown):SessionClientKind{if(value==="web"||value==="desktop"||value==="mobile")return value;throw new AuthenticationFailure("PROVIDER_UNAVAILABLE","La metadata de sesión no es válida.")}
