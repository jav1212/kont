...
import { Currency, type BillingAccount, type OrganizationEntitlements } from "@kontave/billing-domain";
import { organizationId, userId } from "@kontave/organizations-domain";
import { GetBillingOverview, ListBillingPaymentMethods, type BillingAuthorizationContext, type OrganizationBillingAuthorization, type OrganizationBillingRepository } from "../src/index.js";
...
class Repository implements OrganizationBillingRepository { async findAccount() { return account; } async listSubscriptions() { return []; } async getEntitlements() { return entitlements; } async getUsage() { return { companies: { used: 0, maximum: 1, remaining: 1 }, members: { used: 1, maximum: null, remaining: null }, devices: { used: 0, maximum: null, remaining: null } }; } async listInvoices() { return []; } async listPaymentMethods() { return []; } }
