...
import type { DeviceFailure, DeviceSessionEvent } from "@kontave/devices/contracts";
import type { DeviceAdapter, DeviceCandidate, DeviceSession } from "@kontave/devices/core";
import { DelimitedBarcodeParser } from "#devices/node/barcode-parser";
import type { SerialConnection, SerialPortDescriptor, SerialPortProvider } from "#devices/node/serial-port-provider";

const DATALOGIC_VENDOR_ID = "05f9";

/** Discovery and serial configuration for a Datalogic QW2100 scanner. */
...
/** Node serial adapter for Datalogic QuickScan QW2100 barcode scanners. */
export class DatalogicQw2100Adapter implements DeviceAdapter {
  ...
  /**
   * Creates a QW2100 adapter around an injected serial-port boundary.
   * @param ports - Serial provider used for discovery and connections.
   * @param configuration - Optional port identity and baud-rate overrides.
   */
  constructor(
