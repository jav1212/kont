...
/** Minimal SDK boundary required from electron-updater. */
export interface ElectronUpdaterPort {
  autoDownload: boolean;
  autoInstallOnAppQuit: boolean;
  /**
   * Queries the configured update feed.
   * @returns Update metadata when the SDK completes a check, otherwise `null`.
   * @throws An SDK failure when the feed cannot be queried.
   */
  checkForUpdates(): Promise<ElectronUpdateCheckResult | null>;
  /**
   * Downloads the update selected by the SDK.
   * @returns Paths produced by electron-updater after download.
   * @throws An SDK failure when the artifact cannot be downloaded.
   */
  downloadUpdate(): Promise<readonly string[]>;
  /**
   * Quits the application and installs the downloaded update.
   * @param isSilent - Whether the installer should avoid interactive UI.
   * @param isForceRunAfter - Whether the application should restart after installation.
   * @returns Nothing; the process may terminate as an effect.
   */
  quitAndInstall(isSilent?: boolean, isForceRunAfter?: boolean): void;
  /**
   * Registers a download progress listener.
   * @param event - Electron progress event name.
   * @param listener - Listener receiving SDK percentage values.
   * @returns The SDK subscription result.
   */
   ...
   * @param event - Electron progress event name.
   * @param listener - Listener originally passed to `on`.
   * @returns The SDK unsubscription result.
   */
  off(event: "download-progress", listener: (progress: ElectronDownloadProgress) => void): unknown;
}

/** Configuration for the Electron update adapter. */
...
/**
 * Creates the production Electron update provider backed by electron-updater.
 * @param options - Installed release identity and build-specific update policy.
 * @returns A provider ready for use by the update coordinator.
 */
...
/** Electron adapter implementing the portable update provider port. */
export class ElectronClientUpdateProvider implements ClientUpdateProvider {
  ...
  /**
   * Creates an Electron provider and disables implicit SDK downloads and installs.
   * @param updaterPort - Narrow electron-updater SDK boundary.
   * @param options - Installed release identity and adapter configuration.
   */
