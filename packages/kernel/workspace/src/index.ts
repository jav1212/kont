...
export type WorkspaceRelationship =
  typeof WorkspaceRelationship[keyof typeof WorkspaceRelationship];
...
export class WorkspaceContextSession {
  ...
  async select(organizationId: OrganizationId): Promise<ActiveWorkspaceContext> {
    ...
    if (!active) {
      throw new DelegatedAccessFailure("ACCESS_PATH_NOT_FOUND", "El espacio de trabajo no está disponible.");
    }
...
export class ListWorkspacePortfolio {
  ...
  async execute(userId: UserId, occurredAt: string): Promise<readonly WorkspacePortfolioEntry[]> {
    const [direct, delegated] = await Promise.all([
      ...
      this.delegatedAccess.listAssignedToUser(userId),
    ]);
    const portfolio = [
...
export class ResolveWorkspaceAccessPath {
  ...
  async execute(input: {
    ...
    const delegated = await this.delegatedAccess.findAssigned(
      input.userId,
      input.actingOrganizationId,
      input.targetOrganizationId,
    );
    if (delegated && isAssignedAndEffective(delegated, input.occurredAt)) {
      return delegatedPath(input.userId, delegated.grant);
    }
    throw new DelegatedAccessFailure("ACCESS_PATH_NOT_FOUND", "No existe una ruta de acceso vigente.");
  }
}

export class DelegatedPermissionScopePolicy {
  private readonly scopesByPermission = new Map<PermissionCode, DelegatedAccessScope>([
    [permissionCode(PERMISSIONS.ACCOUNTING_READ), DelegatedAccessScope.Accounting],
    [permissionCode(PERMISSIONS.ACCOUNTING_CREATE), DelegatedAccessScope.Accounting],
    [permissionCode(PERMISSIONS.ACCOUNTING_UPDATE), DelegatedAccessScope.Accounting],
    [permissionCode(PERMISSIONS.ACCOUNTING_POST), DelegatedAccessScope.Accounting],
    [permissionCode(PERMISSIONS.ACCOUNTING_CLOSE), DelegatedAccessScope.Accounting],
    [permissionCode(PERMISSIONS.PAYROLL_READ), DelegatedAccessScope.Payroll],
    [permissionCode(PERMISSIONS.PAYROLL_CREATE), DelegatedAccessScope.Payroll],
    [permissionCode(PERMISSIONS.PAYROLL_CONFIRM), DelegatedAccessScope.Payroll],
    [permissionCode(PERMISSIONS.PAYROLL_DELETE), DelegatedAccessScope.Payroll],
    [permissionCode(PERMISSIONS.INVENTORY_READ), DelegatedAccessScope.Inventory],
    [permissionCode(PERMISSIONS.INVENTORY_CREATE), DelegatedAccessScope.Inventory],
    [permissionCode(PERMISSIONS.INVENTORY_UPDATE), DelegatedAccessScope.Inventory],
    [permissionCode(PERMISSIONS.INVENTORY_DELETE), DelegatedAccessScope.Inventory],
    [permissionCode(PERMISSIONS.PURCHASES_READ), DelegatedAccessScope.Purchases],
    [permissionCode(PERMISSIONS.PURCHASES_CREATE), DelegatedAccessScope.Purchases],
    [permissionCode(PERMISSIONS.PURCHASES_CONFIRM), DelegatedAccessScope.Purchases],
    [permissionCode(PERMISSIONS.PURCHASES_CANCEL), DelegatedAccessScope.Purchases],
    [permissionCode(PERMISSIONS.DOCUMENTS_READ), DelegatedAccessScope.Documents],
    [permissionCode(PERMISSIONS.DOCUMENTS_CREATE), DelegatedAccessScope.Documents],
    [permissionCode(PERMISSIONS.DOCUMENTS_UPDATE), DelegatedAccessScope.Documents],
    [permissionCode(PERMISSIONS.DOCUMENTS_DELETE), DelegatedAccessScope.Documents],
  ]);
...
function isAssignedAndEffective(access: AssignedDelegatedAccess, occurredAt: string): boolean {
  return access.assignmentStatus === DelegatedAccessAssignmentStatus.Active
    && isDelegatedAccessEffective(access.grant, occurredAt);
}
...
function selectRestoredWorkspace(
  ...
  return portfolio.find((entry) => entry.relationship === WorkspaceRelationship.Personal)
    ?? portfolio.find((entry) => entry.relationship === WorkspaceRelationship.Member)
    ?? portfolio.find((entry) => entry.relationship === WorkspaceRelationship.Delegated)
    ?? null;
}
...
function delegatedWorkspace(userId: UserId, access: AssignedDelegatedAccess): AccessibleOrganization & { readonly relationship: "delegated" } {
  return {
    relationship: WorkspaceRelationship.Delegated,
    organizationId: access.grant.clientOrganizationId,
    name: access.clientOrganizationName,
    accessPath: delegatedPath(userId, access.grant),
  };
}
...
function delegatedPath(userId: UserId, grant: DelegatedAccessGrant): OrganizationAccessPath {
  return { kind: OrganizationAccessPathKind.DelegatedOrganization, actorUserId: userId, actingOrganizationId: grant.providerOrganizationId, targetOrganizationId: grant.clientOrganizationId, delegationId: grant.id, scopes: grant.scopes };
}
