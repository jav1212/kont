...
import { companyId, organizationId, userId, type CompanyId, type OrganizationId } from "@kontave/organizations/domain";
...
function workspace(id: string, relationship: WorkspaceRelationshipValue): WorkspacePortfolioEntry {
  const organization = organizationId(id);
  const kind = relationship === WorkspaceRelationship.Delegated
...
test("restore preserves an accessible selection", async () => {
  const delegated = workspace("delegated", WorkspaceRelationship.Delegated);
  const direct = workspace("direct", WorkspaceRelationship.Member);
...
test("restore falls back to personal when a stored selection is inaccessible", async () => {
  const delegated = workspace("delegated", WorkspaceRelationship.Delegated);
  const personal = workspace("personal", WorkspaceRelationship.Personal);
...
test("restore selects personal even when it is not alphabetically first", async () => {
  const alphabeticalFirst = workspace("a-member", WorkspaceRelationship.Member);
  const personal = workspace("z-personal", WorkspaceRelationship.Personal);
...
test("restore uses member first when no personal organization exists", async () => {
  const delegated = workspace("a-client", WorkspaceRelationship.Delegated);
  const member = workspace("z-member", WorkspaceRelationship.Member);
...
test("restore uses delegated when no direct relationships exist", async () => {
  const delegated = workspace("client", WorkspaceRelationship.Delegated);
...
test("a direct access remains member unless the backend classifies it as personal", () => {
  ...
  assert.equal(direct.relationship, WorkspaceRelationship.Member);
});
