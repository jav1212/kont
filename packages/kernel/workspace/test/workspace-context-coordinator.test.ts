...
function workspace(id: string, relationship: "personal" | "member" | "delegated"): WorkspacePortfolioEntry {
  ...
  return {
    ...
    relationship,
    accessPath: {
      kind: relationship === WorkspaceRelationship.Delegated
...
test("restore invalidates inaccessible selections and commits one coherent fallback", async () => {
  ...
  const { session } = coordinator({ portfolio: [workspace("a-member", WorkspaceRelationship.Member), personal], store });
...
test("a workspace switch is not visible when persistence fails", async () => {
  const first = workspace("first", WorkspaceRelationship.Personal);
  const second = workspace("second", WorkspaceRelationship.Member);
  const store = new MemoryStore();
