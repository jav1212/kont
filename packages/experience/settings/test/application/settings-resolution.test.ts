...
test("disables entries whose required context is missing", () => {
  const entry = find("organization.general", context({ organizationId: null }));
  assert.equal(entry?.availability, "disabled");
  assert.equal(entry?.unavailableReason, "missing_context");
});
