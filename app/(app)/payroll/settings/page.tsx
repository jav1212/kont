...
export default function PayrollSettingsPage() {
    ...
    return <div className="min-h-full bg-surface-0">
        <PageHeader title={"Configuraci" + String.fromCharCode(243) + "n de n" + String.fromCharCode(243) + "mina"} subtitle={company ? company.name : "Selecciona una empresa"}>
            <BaseButton.Root variant="primary" size="sm" onClick={handleSave} loading={saving} isDisabled={!companyId || loading} leftIcon={!saving ? <Save size={14} /> : undefined}>Guardar cambios</BaseButton.Root>
        </PageHeader>
        <main className="max-w-4xl mx-auto p-4 sm:p-6 space-y-5">
            ...
            {!companyId ? <div className="rounded-xl border border-dashed border-border-light p-10 text-center font-mono text-[12px] text-[var(--text-tertiary)]">Selecciona una empresa para configurar su nomina.</div> : <>
                ...
                <Section title="Conceptos de bonos">
                    ...
                    <div className="space-y-3">{draft.bonusRowDefs.map((bonus, index) => <div key={index} className="grid grid-cols-[auto_1fr_120px_90px_auto] gap-2 items-end"><label className="flex items-center justify-center h-9"><input type="checkbox" checked={bonus.active !== false} onChange={(e) => setBonus(index, { active: e.target.checked })} aria-label="Activar concepto" className="h-4 w-4 accent-[var(--primary)]" /></label><label className="min-w-0"><span className="block mb-1 font-mono text-[10px] uppercase tracking-[0.12em] text-[var(--text-tertiary)]">Nombre</span><input className={inputClass} value={bonus.label} onChange={(e) => setBonus(index, { label: e.target.value })} placeholder="Ej. Ayuda Alimentaria" /></label><label><span className="block mb-1 font-mono text-[10px] uppercase tracking-[0.12em] text-[var(--text-tertiary)]">Monto</span><input className={inputClass + " text-right"} type="number" min={0} step={0.01} value={bonus.amount} onChange={(e) => setBonus(index, { amount: e.target.value })} /></label><label><span className="block mb-1 font-mono text-[10px] uppercase tracking-[0.12em] text-[var(--text-tertiary)]">Moneda</span><select className={inputClass} value={bonus.currency ?? "USD"} onChange={(e) => setBonus(index, { currency: e.target.value as "USD" | "VES" })}><option value="VES">VES</option><option value="USD">USD</option></select></label><button type="button" onClick={() => removeBonus(index)} className="h-9 px-2 rounded-lg border border-border-light text-[var(--text-tertiary)] hover:text-red-500 hover:border-red-500/40" aria-label="Eliminar concepto"><Trash2 size={15} /></button></div>)}</div>
                    <BaseButton.Root variant="secondary" size="sm" onClick={addBonus} leftIcon={<Plus size={14} />}>Agregar concepto</BaseButton.Root>
                </Section>
            </>}
        </main>
    </div>;
