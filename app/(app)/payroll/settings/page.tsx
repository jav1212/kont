        ...
        <main className="max-w-4xl mx-auto p-4 sm:p-6 space-y-5">
            ...
            {!companyId ? <div className="rounded-xl border border-dashed border-border-light p-10 text-center font-mono text-[12px] text-[var(--text-tertiary)]">Selecciona una empresa para configurar su nomina.</div> : <>
                ...
                <Section title="Conceptos de bonos">
                    ...
                    <div className="space-y-3">{draft.bonusRowDefs.map((bonus, index) => <div key={index} className="grid grid-cols-[auto_1fr_120px_90px_auto] gap-2 items-end"><label className="flex items-center justify-center h-9"><input type="checkbox" checked={bonus.active !== false} onChange={(e) => setBonus(index, { active: e.target.checked })} disabled={!canEdit} aria-label="Activar concepto" className="h-4 w-4 accent-[var(--primary)]" /></label><label className="min-w-0"><span className="block mb-1 font-mono text-[10px] uppercase tracking-[0.12em] text-[var(--text-tertiary)]">Nombre</span><input className={inputClass} value={bonus.label} onChange={(e) => setBonus(index, { label: e.target.value })} disabled={!canEdit} placeholder="Ej. Ayuda Alimentaria" /></label><label><span className="block mb-1 font-mono text-[10px] uppercase tracking-[0.12em] text-[var(--text-tertiary)]">Monto</span><input className={inputClass + " text-right"} type="number" min={0} step={0.01} value={bonus.amount} onChange={(e) => setBonus(index, { amount: e.target.value })} disabled={!canEdit} /></label><label><span className="block mb-1 font-mono text-[10px] uppercase tracking-[0.12em] text-[var(--text-tertiary)]">Moneda</span><select className={inputClass} value={bonus.currency ?? "USD"} onChange={(e) => setBonus(index, { currency: e.target.value as PayrollBonusRowDef["currency"] })} disabled={!canEdit}>{<option value="VES">VES</option>}{PAYROLL_REFERENCE_CURRENCIES.map((currency) => <option key={currency.code} value={currency.code}>{currency.code}</option>)}</select></label><button type="button" onClick={() => removeBonus(index)} disabled={!canEdit} className="h-9 px-2 rounded-lg border border-border-light text-[var(--text-tertiary)] hover:text-red-500 hover:border-red-500/40" aria-label="Eliminar concepto"><Trash2 size={15} /></button></div>)}</div>
