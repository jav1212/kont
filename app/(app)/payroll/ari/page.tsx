...
export default function AriPage() {
    ...
    const existing = useMemo(
        () => declarations.find((d) => d.employeeCedula === selectedCedula && d.anioGravable === anio),
        [declarations, selectedCedula, anio],
    );

    // ── Prefill from a saved declaration (render-time guard, sin useEffect) ────
    // La clave incluye declarations.length para re-ejecutar cuando cargan/crean.
    const [loadKey, setLoadKey] = useState("");
    const currentKey = `${selectedCedula}|${anio}|${declarations.length}`;
    if (loadKey !== currentKey) {
        setLoadKey(currentKey);
        if (existing) {
            setValorUT(String(existing.valorUT));
            setRemuneracion(String(existing.remuneracionAnual));
            ...
            // Empleado sin declaración para el año: limpia los campos (conserva U.T.).
            setRemuneracion("");
            setUsarUnico(true);
            setDesgEducacion(""); setDesgSeguros(""); setDesgMedicos(""); setDesgIntereses("");
            setCargas("0");
            setImpuestos("");
        }
    }
    ...
    const input = useMemo(() => ({
        ...
        remuneracionAnual:       toNum(remuneracion),
    ...
    }), [anio, valorUT, remuneracion, usarUnico, desgEducacion, desgSeguros, desgMedicos, desgIntereses, cargas, impuestos]);
    ...
    const handleSave = async () => {
        ...
        if (input.remuneracionAnual <= 0){ notify.error("La remuneración anual estimada debe ser mayor a 0"); return; }
    ...
    const handlePdf = async () => {
        ...
        try {
            ...
            await generateAriPdf({
                ...
                remuneracionAnual:       input.remuneracionAnual,
                usarDesgravamenUnico:    usarUnico,
    ...
    return (
        <div className="min-h-full bg-surface-2 flex flex-col overflow-hidden">
            <PageHeader
                ...
                subtitle="Determinación del porcentaje de retención sobre sueldos y salarios"
                beta
            />
                ...
                <aside className="w-full lg:w-96 shrink-0 flex flex-col border-b lg:border-b-0 lg:border-r border-border-light bg-surface-1 overflow-y-auto">
                    ...
                    <div className="flex-1 divide-y divide-border-light">
                        ...
                        <div className="px-5 py-4 space-y-4">
                            ...
                            <BaseInput.Field
                                label="Remuneración anual estimada (casilla A)"
                                type="number"
                                step={0.01}
                                min={0}
                                value={remuneracion}
                        ...
                        <div className="px-5 py-4 space-y-4">
                            ...
                            <OnlyActiveToggle
                                label="Desgravamen único (774 U.T.)"
                                checked={usarUnico}
                        ...
                        <div className="px-5 py-4 space-y-4">
                            ...
                            <p className={LABEL_CLS + " !normal-case !tracking-normal text-[var(--text-tertiary)]"}>
                                Rebaja personal fija: 10 U.T. + 10 U.T. por cada carga.
                            </p>
                        </div>
                    </div>
                ...
                <main className="flex-1 overflow-y-auto p-6 bg-surface-2">
                    {loading || ariLoading ? (
                        ...
                        <CalculatorEmptyState
                            icon={Receipt}
                            title="Forma AR-I"
                            description="Selecciona un empleado y captura su remuneración anual estimada para determinar el porcentaje de retención de ISLR."
                        />
                    ) : (
                        <div className="max-w-3xl mx-auto space-y-6">
                            <ConstanciaShell
                                ...
                                headerRight={
                                    <>
                                        <p className="font-mono text-[9px] uppercase tracking-[0.2em] text-[var(--text-tertiary)] mb-1">Año Gravable</p>
                                ...
                                kpis={[
                                    ...
                                    { label: "Remuneración anual", value: formatCurrency(input.remuneracionAnual) },
                                    { label: "Desgravamen",        value: usarUnico ? "Único (774 U.T.)" : "Detallado" },
                                    { label: "Cargas familiares",  value: String(input.cargasFamiliares) },
                                ]}
                                documentId={makeDocumentId(company?.name, selectedEmp.cedula, anio)}
                            ...
                            >
                                ...
                                <div className="space-y-1">
                                    <CalcRow
                                        ...
                                        formula={`${formatCurrency(input.remuneracionAnual)} ÷ ${formatCurrency(input.valorUT)}`}
                                ...
                                {/* Panel del porcentaje resultante (casilla J) */}
                                <div className="mt-5 p-5 rounded-2xl bg-primary-500/[0.06] border border-primary-500/20 flex items-end justify-between gap-4">
                                    ...
                                    <p className="font-mono text-[10px] text-[var(--text-tertiary)] text-right max-w-[45%] leading-relaxed">
                                        Se aplica sobre cada pago o abono en cuenta del año gravable.
                                    </p>
                                </div>

                                {!result.sujetoARetencion && input.remuneracionAnual > 0 && (
                                    <div className="mt-4 flex items-center gap-3 bg-amber-500/5 border border-amber-500/30 rounded-lg px-4 py-3">
                                        ...
                                        <p className="font-mono text-[11px] text-amber-700">
                                            Remuneración anual &lt; 1.000 U.T. — el trabajador no está sujeto a retención.
                                        </p>
                                    </div>
                                )}
                            </ConstanciaShell>

                            {/* Declaraciones guardadas de la empresa */}
                            {declarations.length > 0 && (
                                <div className="bg-surface-1 rounded-2xl border border-border-light overflow-hidden">
                                    ...
                                    <div className="divide-y divide-border-light/60">
                                        {declarations.map((d) => (
                                            <button
                                                key={d.id ?? `${d.employeeCedula}-${d.anioGravable}`}
                                                type="button"
                                                onClick={() => { setSelectedCedula(d.employeeCedula); setAnio(d.anioGravable); }}
                                            ...
                                            >
                                                <div className="min-w-0">
                                                    ...
                                                    <span className="font-mono text-[11px] text-[var(--text-tertiary)] tabular-nums">{d.employeeCedula} · {d.anioGravable}</span>
                                                </div>
