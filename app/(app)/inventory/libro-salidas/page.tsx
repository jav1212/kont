...
function exportCSV(rows: LibroVentasRow[], periodo: string) {
    const headers = [
        ...
        "Base Exenta", "Base Grav. 8%", "IVA 8%", "Base Grav. 16%", "IVA 16%",
        "Autoconsumo", "IVA Autoconsumo", "Total", "Tipo",
    ];
    const lines = [
        headers.join(","),
        ...rows.map((r) => [
...
export default function LibroVentasPage() {
    ...
    const totales = useMemo(() => ({
        ...
        baseGravada8:   sum(libroVentas, "baseGravada8"),
        iva8:           sum(libroVentas, "iva8"),
        baseGravada16:  sum(libroVentas, "baseGravada16"),
        iva16:          sum(libroVentas, "iva16"),
        autoconsumo:    sum(libroVentas, "autoconsumo"),
        ivaAutoconsumo: sum(libroVentas, "ivaAutoconsumo"),
        total:          sum(libroVentas, "total"),
    }), [libroVentas]);

    const debitoFiscal = totales.iva8 + totales.iva16 + totales.ivaAutoconsumo;
    ...
    return (
        <div className="min-h-full bg-surface-2 font-mono">
                    ...
                    <div className="flex items-center gap-3">
                        <div className="flex items-center gap-2">
                            ...
                            <input
                                ...
                                className="h-9 px-2 rounded-lg border border-border-light bg-surface-1 text-[13px] text-foreground outline-none focus:border-primary-500/60"
                            />
                        </div>
                        <button
                        ...
                        <button
                            ...
                            className="h-9 px-4 rounded-lg border border-red-500/30 bg-red-500/[0.06] hover:bg-red-500/[0.12] text-red-500 text-[12px] uppercase tracking-[0.12em] transition-colors"
                        >
                    ...
                    <div className="bg-surface-1 border border-border-light rounded-xl shadow-xl w-full max-w-md mx-4">
                        <div className="px-6 py-4 border-b border-border-light flex items-center justify-between">
                            ...
                            <button
                                ...
                                className="text-[var(--text-tertiary)] hover:text-foreground text-[13px] uppercase tracking-[0.10em]"
                            >
                                ✕
                            </button>
                        </div>
            ...
            <div className="px-8 py-6">
                ...
                {loadingLibroVentas ? (
                    <div className="py-16 text-center text-[13px] text-[var(--text-tertiary)]">Cargando libro de ventas…</div>
                ) : libroVentas.length === 0 ? (
                    ...
                    <div className="space-y-4">
                        <div className="rounded-xl border border-border-light bg-surface-1 overflow-hidden">
                                ...
                                <table className="w-full text-[12px] whitespace-nowrap">
                                    <thead>
                                        <tr className="border-b border-border-light bg-surface-2">
                                            {[
                                                ...
                                                { label: "Base Grav. 8%",  align: "right", width: "min-w-[100px]" },
                                                { label: "IVA 8%",         align: "right", width: "min-w-[85px]"  },
                                                { label: "Base Grav. 16%", align: "right", width: "min-w-[105px]" },
                                                { label: "IVA 16%",        align: "right", width: "min-w-[85px]"  },
                                                ...
                                                <th key={col.label}
                                                    className={`px-3 py-2.5 text-[11px] uppercase tracking-[0.10em] text-[var(--text-tertiary)] font-normal text-${col.align} ${col.width}`}>
                                                    {col.label}
                                                </th>
                                            ))}
                                        </tr>
                                    ...
                                    <tbody>
                                        ...
                                        <tr className="border-t-2 border-primary-500/30 bg-primary-500/[0.04]">
                                            <td className="px-3 py-2.5 text-[12px] uppercase tracking-[0.10em] font-bold text-foreground" colSpan={4}>
                                                Total del período
                                            </td>
                                            <td className="px-3 py-2.5 tabular-nums text-right text-[12px] font-bold text-foreground">{fmtN(totales.baseExenta)}</td>
                                            <td className="px-3 py-2.5 tabular-nums text-right text-[12px] font-bold text-foreground">{fmtN(totales.baseGravada8)}</td>
                                            <td className="px-3 py-2.5 tabular-nums text-right text-[12px] font-bold text-foreground">{fmtN(totales.iva8)}</td>
                                            <td className="px-3 py-2.5 tabular-nums text-right text-[12px] font-bold text-foreground">{fmtN(totales.baseGravada16)}</td>
                                            <td className="px-3 py-2.5 tabular-nums text-right text-[12px] font-bold text-foreground">{fmtN(totales.iva16)}</td>
                                            <td className="px-3 py-2.5 tabular-nums text-right text-[12px] font-bold text-amber-500">{fmtN(totales.autoconsumo)}</td>
                                            <td className="px-3 py-2.5 tabular-nums text-right text-[12px] font-bold text-amber-500">{fmtN(totales.ivaAutoconsumo)}</td>
                                            <td className="px-3 py-2.5 tabular-nums text-right text-[12px] font-bold text-foreground">{fmtN(totales.total)}</td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
                            ...
                            <div className="px-4 py-3 border-t border-border-light bg-surface-2 flex items-center gap-6 flex-wrap">
                                <span className="text-[11px] uppercase tracking-[0.12em] text-[var(--text-tertiary)] font-medium">
                                ...
                                <div className="flex items-center gap-4 text-[11px] text-[var(--text-tertiary)]">
                                    {totales.iva16 > 0 && (
                                        <span>
                                            IVA 16%: <span className="tabular-nums text-foreground font-medium">{fmtN(totales.iva16)}</span>
                                        </span>
                                    )}
                                    {totales.iva8 > 0 && (
                                        <span>
                                            IVA 8%: <span className="tabular-nums text-foreground font-medium">{fmtN(totales.iva8)}</span>
                                        </span>
                                    )}
