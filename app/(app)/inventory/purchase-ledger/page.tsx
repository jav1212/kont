...
import { generatePurchaseLedgerPdf } from "@/src/modules/inventory/frontend/utils/purchase-ledger-pdf";
...
type EntryFilter = "all" | "entrada" | "devolucion_salida";
...
function TypeBadge({ type }: { type: MovementType }) {
    const isReturn = type === "devolucion_salida";
    const Icon = isReturn ? Undo2 : PackagePlus;
    const cls = isReturn
        ? "border-amber-500/40 text-amber-600 bg-amber-500/[0.06]"
...
const TYPE_OPTIONS: { value: EntryFilter; label: string }[] = [
    { value: "all",               label: "Todas" },
    { value: "entrada",           label: "Entradas" },
    { value: "devolucion_salida", label: "Dev. cliente" },
];
...
function exportCSV(rows: Movement[], productNameById: Map<string, string>, period: string) {
    ...
    a.href = url;
    a.download = `libro-entradas-${period}.csv`;
    a.click();
            ...
            <div className="bg-surface-1 border border-border-medium rounded-xl shadow-xl w-full max-w-md mx-4">
                <div className="px-6 py-4 border-b border-border-light">
                    ...
                    <p className="mt-1 font-sans text-[12px] text-[var(--text-tertiary)] leading-snug">
                        Entrada de <strong className="text-foreground">{productName}</strong>. Solo se editan fecha, referencia y notas — el saldo no cambia.
                    </p>
                </div>
                <div className="px-6 py-5 space-y-4">
                    ...
                    <BaseInput.Field
                        ...
                        placeholder="Nº factura, motivo o referencia interna…"
                    />
                    <BaseInput.Field
                        label="Notas"
                        type="text"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
            <div className="bg-surface-1 border border-yellow-500/20 rounded-xl shadow-xl w-full max-w-md mx-4">
                ...
                <div className="px-6 py-5 font-sans text-[14px] text-foreground leading-relaxed">
                    <p>
                        Vas a eliminar la entrada de <strong>{productName}</strong> del{" "}
                    ...
                    <p className="mt-3 text-[13px] text-yellow-700 font-medium">
                        La existencia del producto se reducirá en {fmtN0(mov.quantity)} unidades. ¿Continuar?
                    </p>
                </div>
                <div className="px-6 py-4 border-t border-border-light flex items-center justify-end gap-3">
...
export default function LibroEntradasPage() {
    ...
    const inPeriod = useMemo(
        () => movements
            .filter((m) => (m.type === "entrada" || m.type === "devolucion_salida") && m.period === period)
            .sort((a, b) => (b.date ?? "").localeCompare(a.date ?? "")),
        [movements, period],
    );

    const counts = useMemo<Record<EntryFilter, number>>(() => ({
        all:               inPeriod.length,
        entrada:           inPeriod.filter((m) => m.type === "entrada").length,
        devolucion_salida: inPeriod.filter((m) => m.type === "devolucion_salida").length,
    }), [inPeriod]);
            ...
            <div className="px-8 py-6 space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <DashboardKpiCard
                        ...
                        icon={Boxes}
                        loading={loading}
                        sublabel={totals.qty === 0 ? "ninguna unidad ingresada" : "unidades ingresadas"}
                    />
                    <DashboardKpiCard
                        label="Costo total"
                        ...
                        icon={Coins}
                        loading={loading}
                        sublabel="valor total de las entradas"
                    />
                    <DashboardKpiCard
                        label="Productos"
                    ...
                    {loading ? (
                        ...
                        <div className="px-5 py-16 flex flex-col items-center justify-center gap-3 text-center">
                            ...
                            <p className="font-sans text-[13px] text-[var(--text-tertiary)] max-w-md">
                                Las entradas se registran al confirmar facturas de compra o devoluciones de cliente. Crea una para empezar.
                            </p>
                            <div className="flex items-center gap-2 mt-2">
                                ...
                                <BaseButton.Root
                                    ...
                                    variant="secondary"
                                    size="sm"
                                    leftIcon={<Undo2 size={13} strokeWidth={2} />}
                                >
                ...
                <div className="flex items-center justify-between pt-2 pb-4 font-sans text-[12px] text-[var(--text-tertiary)]">
                    <span>
                        Las entradas también aparecen en el listado completo de{" "}
