...
import { ProductCompositionEditor } from "@/src/modules/inventory/frontend/components/product-composition-editor";
...
export default function ProductDetailPage() {
    ...
    const [savedCompositionKind, setSavedCompositionKind] = useState<"simple" | "composite" | null>(null);
    ...
    async function handleSave() {
        ...
        setSaving(false);
        if (saved) { notify.success("Producto actualizado"); setForm(saved); setSavedCompositionKind(saved.compositionKind ?? "simple"); }
    }

    const display = form ?? source;
    const persistedCompositionKind = savedCompositionKind ?? source?.compositionKind ?? "simple";
    const canEditComposition = persistedCompositionKind === "composite";
            ...
            <main className="mx-auto flex w-full max-w-6xl flex-col space-y-6 px-2.5 py-6 sm:px-4 lg:px-5">
                {loadingProducts || loadingDepartments ? <div className="rounded-xl border border-border-light bg-surface-1 p-8 font-sans text-[13px] text-[var(--text-secondary)]">Cargando producto…</div> : !display ? <div className="rounded-xl border border-border-light bg-surface-1 p-8 font-sans text-[13px] text-[var(--text-secondary)]">Producto no encontrado.</div> : (
                    <>
                        ...
                        <Section icon={Boxes} title="Clasificación" description="Tipo, unidad, valuación y composición."><div className="grid gap-4 md:grid-cols-4"><div><label className={labelCls}>Tipo</label><select className={fieldCls} value={form?.type ?? "mercancia"} onChange={(e) => set("type", e.target.value as ProductType)}><option value="mercancia">Mercancía</option></select></div><div><label className={labelCls}>Unidad de medida</label><select className={fieldCls} value={form?.measureUnit ?? "unidad"} onChange={(e) => set("measureUnit", e.target.value as MeasureUnit)}>{units.map((u) => <option key={u.value} value={u.value}>{u.label}</option>)}</select></div><div><label className={labelCls}>Método de valuación</label><select className={fieldCls} value={form?.valuationMethod ?? "promedio_ponderado"} onChange={(e) => set("valuationMethod", e.target.value as ValuationMethod)}><option value="promedio_ponderado">Promedio ponderado</option></select></div><div><label className={labelCls}>Presentación</label><select className={fieldCls} value={form?.compositionKind ?? "simple"} onChange={(e) => set("compositionKind", e.target.value as "simple" | "composite")}><option value="simple">Producto simple</option><option value="composite">Producto compuesto</option></select></div></div></Section>
                        {form?.compositionKind === "composite" && !canEditComposition && <Section icon={Boxes} title="Composición" description="Los componentes se descontarán al confirmar la venta."><p className="text-[13px] text-[var(--text-secondary)]">Guarda el cambio a producto compuesto antes de definir los componentes.</p></Section>}
                        {canEditComposition && form && <Section icon={Boxes} title="Composición" description="Los componentes se descontarán al confirmar la venta.">{form.compositionKind === "simple" && <p className="mb-4 rounded-lg border border-amber-500/20 bg-amber-500/5 px-3 py-2 text-[12px] text-amber-700">Para convertir este producto en simple, primero elimina todos los componentes y guarda la composición. Después guarda el cambio de presentación.</p>}<ProductCompositionEditor companyId={companyId ?? ""} product={form} products={products} onSaved={async () => { if (!companyId) return; const refreshed = await loadProducts(companyId, true); const updated = refreshed?.find((product) => product.id === form.id); if (updated) setForm((current) => current ? { ...current, components: updated.components, compositionStatus: updated.compositionStatus } : updated); }} /></Section>}
