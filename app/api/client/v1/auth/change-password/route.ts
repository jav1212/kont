...
export async function POST(request: Request) {
  return executeSecurityRequest(
    request,
    async ({ actions, userId, sessionId, accessToken }) => {
...
async function readChangePasswordBody(
  ...
  if (
    ...
    ((value as { revokeOtherSessions?: unknown }).revokeOtherSessions !==
      undefined &&
      typeof (value as { revokeOtherSessions?: unknown })
        .revokeOtherSessions !== "boolean")
  ) {
    throw new AuthenticationFailure(
      "INVALID_INPUT",
      "La solicitud para cambiar la contraseña no es válida.",
    );
  }
