...
export async function GET(request: Request, c: C) {
  const { organizationId } = await c.params,
    value = new URL(request.url).searchParams.get("companyId");
  return executeDocumentRequest(
    ...
    async (a, organization) => {
      ...
      if (value !== null)
        input.companyId = value === "organization" ? null : companyId(value);
      return (await a.listFolders.execute(input)).map(toDocumentFolderDto);
    },
  );
}
export async function POST(request: Request, c: C) {
  const { organizationId } = await c.params,
    b = (await request.json()) as CreateDocumentFolderDto;
  return executeDocumentRequest(
    request,
    organizationId,
    documentsCreate,
    async (a, organization, actor) =>
      toDocumentFolderDto(
        await a.createFolder.execute({
          organizationId: organization,
          companyId: b.companyId ? companyId(b.companyId) : null,
          parentId: b.parentId ? documentFolderId(b.parentId) : null,
          name: b.name,
          createdBy: actor,
        }),
      ),
