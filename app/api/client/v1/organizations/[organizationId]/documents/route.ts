...
export async function GET(request: Request, c: C) {
  const { organizationId } = await c.params,
    ...
    company = url.searchParams.get("companyId"),
    folder = url.searchParams.get("folderId");
  return executeDocumentRequest(
    ...
    async (a, organization) => {
      const input: {
        ...
        companyId?: ReturnType<typeof companyId> | null;
        folderId?: ReturnType<typeof documentFolderId> | null;
      } = { organizationId: organization };
      if (company !== null)
        input.companyId =
          company === "organization" ? null : companyId(company);
      if (folder !== null)
        input.folderId = folder === "root" ? null : documentFolderId(folder);
      return (await a.listDocuments.execute(input)).map(toDocumentDto);
    },
  );
}
export async function POST(request: Request, c: C) {
  const { organizationId } = await c.params,
    b = (await request.json()) as RegisterDocumentDto;
  return executeDocumentRequest(
    request,
    organizationId,
    documentsCreate,
    async (a, organization, actor) =>
      toDocumentDto(
        await a.register.execute({
          organizationId: organization,
          companyId: b.companyId ? companyId(b.companyId) : null,
