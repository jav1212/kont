...
import { toManualPaymentRequestDto } from "@/src/client-api/v1/billing/billing-mapper";
...
export async function GET(request: Request, context: Context) {
  ...
  return executeBillingRequest(
    ...
    async (actor, organization, authorization) =>
      (
        await createBillingActions().manualPaymentRequests.execute(
          actor,
          organization,
          authorization,
        )
  ...
  return executeBillingRequest(
    ...
    async (actor, organization, authorization) => {
      ...
      return toManualPaymentRequestDto(
        await createBillingActions().submitManualPaymentRequest.execute(
          actor,
          organization,
          {
            planId: body.planId,
...
async function readBody(
  request: Request,
): Promise<SubmitManualPaymentRequestDto> {
  let value: unknown;
  try {
  ...
  const body = value as Partial<SubmitManualPaymentRequestDto> | null;
  if (
    !body ||
    ...
    (body.receiptStorageKey !== undefined &&
      body.receiptStorageKey !== null &&
      typeof body.receiptStorageKey !== "string")
  )
    throw invalidRequest();
  return body as SubmitManualPaymentRequestDto;
}
