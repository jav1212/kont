import type { ResendMemberInvitationDto } from "@kontave/client-contracts";
import {
...
export async function POST(request: Request, context: Context) {
  ...
  const body = (await request.json()) as ResendMemberInvitationDto;
  const days = Math.min(Math.max(body.expiresInDays ?? 7, 1), 30);
  return executeMemberRequest(
    request,
    organizationId,
    membersInvite,
    async (actions, actor, organization, identity) => {
