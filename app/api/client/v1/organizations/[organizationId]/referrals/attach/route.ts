...
import { toReferralAttributionDto } from "@/src/client-api/v1/referrals/referral-dto";
import { z } from "zod";

export const dynamic = "force-dynamic";

const attachReferralRequestSchema = z.object({
    ...
    return Response.json(
      {
        error: {
          ...
          message: "El código de referido es inválido.",
        },
      },
      { status: 400 },
    );
  ...
  return executeReferralRequest(
    ...
    async (organization) =>
      toReferralAttributionDto(
        await createReferralActions().attach.execute(
          organization,
          payload.code,
        ),
      ),
