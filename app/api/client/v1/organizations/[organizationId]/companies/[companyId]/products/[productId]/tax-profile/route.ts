...
export async function GET(request: Request, c: C) {
  ...
  return executeProductRequest(
    ...
    async (_a, context) =>
      dto(
        await createProductCommercialActions().getTaxation.execute(
          context,
...
export async function PATCH(request: Request, c: C) {
  const p = await c.params,
    b = (await request.json()) as UpdateProductTaxationDto;
  return executeProductRequest(
    request,
    p.organizationId,
    p.companyId,
    productsUpdate,
    async (_a, context) => {
      const actions = createProductCommercialActions(),
        id = productId(p.productId);
      await actions.updateTaxation.execute({
        ...context,
        productId: id,
      ...
      return dto(
        await actions.getTaxation.execute(
          context,
          id,
          VENEZUELAN_IVA,
          b.effectiveFrom,
...
function dto(value: ResolvedProductTaxation) {
  return {
    profileId: value.profile.id,
    taxCode: value.assignment.taxCode,
    treatment: value.assignment.treatment,
    effectiveFrom: value.assignment.effectiveFrom,
    effectiveTo: value.assignment.effectiveTo,
    resolvedRate: value.rule.rate,
    legalBasis: value.assignment.legalBasis,
    ruleVersion: `${value.assignment.classificationVersion}|${value.rule.version}`,
