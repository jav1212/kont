...
export async function GET(request: Request, c: C) {
  const p = await c.params,
    ...
    to = url.searchParams.get("to") ?? caracasDate(),
    from = url.searchParams.get("from") ?? yearBefore(to),
  ...
  return executeProductRequest(
    ...
    productsRead,
    async (_a, context) =>
      createProductCommercialActions().unitEconomics.execute({
        ...context,
...
function yearBefore(value: string) {
