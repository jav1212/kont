...
import { toProductCategoryOverviewDto } from "@/src/client-api/v1/products/product-mapper";
type Context = {
...
export async function GET(request: Request, routeContext: Context) {
  ...
  return executeProductRequest(
    ...
    async (actions, context) =>
      toProductCategoryOverviewDto(
        await actions.categoryOverview.execute({
          ...context,
          search: url.searchParams.get("search"),
          status: one(url, "status", ["active", "inactive", "all"], "all") as
            | ProductCategoryStatus
            | "all",
          sort: one(url, "sort", ["name", "products", "updatedAt"], "name"),
          direction: one(url, "direction", ["asc", "desc"], "asc"),
...
function limit(url: URL) {
  ...
  if (!Number.isSafeInteger(value))
    throw new ProductFailure("PRODUCT_CATEGORY_INVALID", "limit no es válido.");
  return value;
}
