import type { CreateInventoryOperationDto } from "@kontave/client-contracts";
import {
...
export async function GET(
  ...
  return executeInventoryOperationRequest(
    ...
    p.companyId,
    inventoryRead,
    (a, c) => a.list.execute({ ...c, ...inventoryFlowQuery(request) }),
  );
}
  ...
  return executeInventoryOperationRequest(
    ...
    async (a, c) =>
      a.create.execute({
        ...
        ...((await readJson(
          request,
        )) as unknown as CreateInventoryOperationDto),
      }),
  );
}
