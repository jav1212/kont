import type { UpdateProductDto } from "@kontave/client-contracts";
import {
  barcode,
  productCategoryId,
  productId,
  ProductFailure,
  sku,
  UnitOfMeasure,
} from "@kontave/products/domain";
import {
  executeProductRequest,
  productsRead,
  productsUpdate,
} from "@/src/client-api/v1/products/product-http";
import { toProductDetailDto } from "@/src/client-api/v1/products/product-mapper";
import { createProductCommercialActions } from "@/src/client-api/v1/products/product-commercial-actions";
import { VENEZUELAN_IVA } from "@kontave/taxation/venezuela";
type C = {
  params: Promise<{
    organizationId: string;
    companyId: string;
    productId: string;
  }>;
};
export async function GET(request: Request, c: C) {
  const p = await c.params,
    url = new URL(request.url);
  return executeProductRequest(
    request,
    p.organizationId,
    p.companyId,
    productsRead,
    async (a, context) => {
      const id = productId(p.productId),
        commercial = createProductCommercialActions(),
        date = url.searchParams.get("effectiveDate") ?? caracasDate();
      const [product, pricing, taxation] = await Promise.all([
        a.get.execute(context, id),
        commercial.getPricing.execute(context, id),
        commercial.getTaxation.execute(context, id, VENEZUELAN_IVA, date),
      ]);
      return toProductDetailDto(product, { pricing, taxation });
    },
  );
}
export async function PATCH(request: Request, c: C) {
  const p = await c.params,
    b = (await request.json()) as UpdateProductDto;
  if (!Number.isSafeInteger(b.expectedVersion))
    throw new ProductFailure(
      "PRODUCT_INVALID",
      "expectedVersion es requerido.",
    );
  return executeProductRequest(
    request,
    p.organizationId,
    p.companyId,
    productsUpdate,
    async (a, context) =>
      toProductDetailDto(
        await a.update.execute({
          ...context,
          productId: productId(p.productId),
          expectedVersion: b.expectedVersion,
          ...(b.sku === undefined ? {} : { sku: sku(b.sku) }),
          ...(b.barcodes === undefined
            ? {}
            : { barcodes: b.barcodes.map(barcode) }),
          ...(b.name === undefined ? {} : { name: b.name }),
          ...(b.description === undefined
            ? {}
            : { description: b.description }),
          ...(b.categoryId === undefined
            ? {}
            : {
                categoryId:
                  b.categoryId === null
                    ? null
                    : productCategoryId(b.categoryId),
              }),
          ...(b.baseUnit === undefined ? {} : { baseUnit: unit(b.baseUnit) }),
        }),
      ),
  );
}
function unit(value: string) {
  if (!Object.values(UnitOfMeasure).includes(value as UnitOfMeasure))
    throw new ProductFailure("PRODUCT_INVALID", "baseUnit no es válida.");
  return value as UnitOfMeasure;
}
function caracasDate() {
  return new Intl.DateTimeFormat("en-CA", {
    timeZone: "America/Caracas",
    year: "numeric",
    month: "2-digit",
    day: "2-digit",
  }).format(new Date());
}
