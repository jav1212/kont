...
export async function GET(request: Request, c: C) {
  ...
  return executeProductRequest(
    ...
    async (_a, context) =>
      dto(
        await createProductCommercialActions().getPricing.execute(
          context,
...
export async function PATCH(request: Request, c: C) {
  const p = await c.params,
    b = (await request.json()) as UpdateProductSalePricingDto;
  return executeProductRequest(
    request,
    p.organizationId,
    p.companyId,
    ...
    async (_a, context) =>
      dto(
        await createProductCommercialActions().updatePricing.execute(
          context,
          productId(p.productId),
          policy(b.policy),
          b.expectedVersion,
...
function policy(value: UpdateProductSalePricingDto["policy"]) {
  if (value === null) return null;
  if (value.mode === "fixed")
    return fixedSalePricing(value.amount, currencyCode(value.currency));
  if (value.mode === "markup")
    return markupSalePricing(value.percentage, currencyCode(value.currency));
  throw new PricingFailure("PRICING_INVALID", "Pricing mode is invalid.");
}
function dto(value: ProductSalePricing) {
  return {
    policy: value.policy,
