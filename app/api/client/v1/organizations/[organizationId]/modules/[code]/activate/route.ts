...
export async function POST(
  request: Request,
  context: { params: Promise<{ organizationId: string; code: string }> },
) {
  const { organizationId, code } = await context.params;
  const parsed = z.enum(ModuleCode).safeParse(code);
  if (!parsed.success)
    return Response.json(
      { error: { code: "INVALID_REQUEST", message: "El módulo es inválido." } },
      { status: 400 },
    );
  return executeModuleRequest(
    ...
    true,
    async (organization) =>
      toModuleInstallationDto(
        await createModuleActions().activate.execute(
          organization,
          parsed.data,
