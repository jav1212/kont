...
export async function GET(request: Request, c: C) {
  ...
  return executeRoleRequest(request, p.organizationId, rolesRead, async (a) =>
    (await a.listRoles.execute(p.organizationId)).map(toRoleDto),
  );
}
export async function POST(request: Request, c: C) {
  const p = await c.params,
    b = (await request.json()) as CreateRoleDto,
