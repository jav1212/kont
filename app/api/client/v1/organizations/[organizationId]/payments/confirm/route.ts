...
import { toPaymentDto } from "@/src/client-api/v1/payments/payment-dto";
...
export async function POST(
    ...
    return Response.json(
      {
        error: {
          ...
          message: "Los datos del pago son inválidos.",
        },
      },
      { status: 400 },
    );
  ...
  return executePaymentRequest(
    ...
    async (organization) =>
      toPaymentDto(
        await createPaymentActions().confirm.execute({
          organizationId: organization,
          invoiceId: body.invoiceId,
          provider: body.provider,
          providerReference: body.providerReference,
          amount: money(BigInt(body.amountMinor), body.currency),
