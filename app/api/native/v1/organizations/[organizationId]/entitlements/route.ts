...
import { toEntitlementsDto } from "@/src/native-api/v1/billing/billing-mapper";
...
export async function GET(request: Request, context: { params: Promise<{ organizationId: string }> }) {
  ...
  return executeBillingRequest(request, organizationId, async (actor, organization, authorization) =>
    toEntitlementsDto(await createBillingActions().entitlements.execute(actor, organization, authorization)));
}
