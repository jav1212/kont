...
export const GET = withTenant(async (req, { userId, actingAs }) => {
    ...
    const result = await getInventoryActions(ownerId).listTransformations.execute({ companyId });
...
export const POST = withTenant(async (req, { userId, actingAs }) => {
    ...
    const ownerId = actingAs?.ownerId ?? userId;
    const result = await getInventoryActions(ownerId).saveTransformation.execute(body);
    return handleResult(result);
});
