...
export const GET = withTenant(async (req, { userId }) => {
    ...
    const result = await getInventoryActions(userId).listPurchaseInvoices.execute({ companyId });
...
export const POST = withTenant(async (req, { userId }) => {
    ...
    const result = await getInventoryActions(userId).savePurchaseInvoice.execute({ invoice, items });
    return handleResult(result);
});
