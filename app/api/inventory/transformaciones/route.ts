...
export const GET = withTenant(async (req, { userId }) => {
    ...
    const result = await getInventoryActions(userId).getTransformaciones.execute({ empresaId });
    return handleResult(result);
});

export const POST = withTenant(async (req, { userId }) => {
    const body = await req.json();
    const result = await getInventoryActions(userId).saveTransformacion.execute(body);
    return handleResult(result);
});
