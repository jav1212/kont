// API route for the inventory purchase ledger (libro de entradas).
...
export const GET = withTenant(async (req, { userId, actingAs, effectiveOwnerId, tenantId}) => {
    ...
    const result = await getInventoryActions(tenantId).getPurchaseLedger.execute({ companyId, period });
