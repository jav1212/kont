...
export const GET = withTenant(async (req, { userId }) => {
    ...
    const result = await getInventoryActions(userId).getDepartamentos.execute({ empresaId });
    return handleResult(result);
});

export const POST = withTenant(async (req, { userId }) => {
    const body = await req.json();
    const result = await getInventoryActions(userId).saveDepartamento.execute(body);
    return handleResult(result);
});
