...
export const GET = withTenant(async (req, { userId }) => {
    ...
    const result = await getInventoryActions(userId).listDepartments.execute({ companyId });
    return handleResult(result);
});

export const POST = withTenant(async (req, { userId }) => {
    const body = await req.json();
    const result = await getInventoryActions(userId).saveDepartment.execute(body);
    return handleResult(result);
});
