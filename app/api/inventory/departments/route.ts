// API route for inventory departments.
...
export const GET = withTenant(async (req, { userId, actingAs, effectiveOwnerId, tenantId}) => {
    ...
    const result = await getInventoryActions(tenantId).listDepartments.execute({ companyId });
...
export const POST = withTenant(async (req, { userId, actingAs, effectiveOwnerId, tenantId}) => {
    ...
    const ownerId = effectiveOwnerId;
    const result = await getInventoryActions(tenantId).saveDepartment.execute(body);
    return handleResult(result);
});
