...
export const GET = withTenant(async (req, { userId }) => {
    ...
    const result = await getInventoryActions(userId).listMovements.execute({ companyId, period });
    return handleResult(result);
});

export const POST = withTenant(async (req, { userId }) => {
    const body = await req.json();
    const result = await getInventoryActions(userId).saveMovement.execute(body);
    return handleResult(result);
});
