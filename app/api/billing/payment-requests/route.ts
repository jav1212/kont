 ...
 */
export const GET = withTenantPermission('billing.read', async (_req, { userId, actingAs, effectiveOwnerId}) => {
    const tenantId = effectiveOwnerId;
    const result = await getBillingActions().getPaymentRequests.execute({ tenantId });
    return handleResult(result);
});

export const POST = withTenantPermission('billing.manage', async (req, { userId, actingAs, effectiveOwnerId}) => {
