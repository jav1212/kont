...
// ============================================================================
// ESTADO BADGE — sobrio, sin dots, solo texto con fondo sutil
// ============================================================================
...
// ============================================================================
// STAT — métrica inline del summary bar
// ============================================================================
...
const Stat = ({ label, value, muted }: { label: string; value: string | number; muted?: boolean }) => (
    <div className="flex flex-col gap-0.5">
        ...
        <span className={[
            ...
            muted ? "text-neutral-500 dark:text-neutral-400" : "text-foreground",
        ].join(" ")}>
            {value}
        </span>
    </div>
...
export default function PayrollTablePage() {
  ...
  const searchCols  = useMemo(() => {
    ...
    return raw
      ? new Set<string | number>(raw.split(",").filter(Boolean))
  ...
  const handleColsChange = useCallback(
    (cols: Set<string | number>) =>
      updateParams({ cols: Array.from(cols).join(",") || null }),
    [updateParams]
  );
    ...
    const columns: Column<Employee>[] = useMemo(() => [
        {
            ...
            width: 148,
            render: (val) => (
                <span className="font-mono text-[12px] text-neutral-500 dark:text-neutral-400 tabular-nums tracking-wide">
                    {val}
                </span>
            ),
        },
        {
            ...
            render: (val, item) => (
                <div className="flex flex-col min-w-0">
                    ...
                    <span className="font-mono text-[10px] text-neutral-400 dark:text-neutral-500 uppercase tracking-wider mt-[3px]">
                        {item.cargo}
                    </span>
                </div>
            ),
        ...
        },
        {
            ...
            sortable: true,
            render: (val) => (
                <span className="font-mono text-[12px] text-neutral-600 dark:text-neutral-400 tabular-nums">
                    Bs. {fmt(Number(val))}
                </span>
            ),
        },
        {
            ...
            sortable: true,
            render: (val) => (
                <span className="font-mono text-[12px] text-error/70 tabular-nums">
                    − {fmt(Number(val))}
                </span>
            ),
        },
        ...
        },
        {
            ...
            label: "Estado",
            align: "center",
            render: (val) => <EstadoBadge estado={val as Employee["estado"]} />,
        },
    ], []);

    // ── sorted data ───────────────────────────────────────────────────────────
    const sortedEmpleados = useMemo(
        () => sortedData(empleadosEjemplo),
        [sortedData]
    );
        ...
        <div className="min-h-screen bg-background px-6 py-8">
            <div className="max-w-6xl mx-auto space-y-5">
                ...
                <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 pb-1">
                    ...
                    <BaseButton.Root variant="primary" size="sm">
                        + Nuevo
                    </BaseButton.Root>
                </div>
                ...
                <div className={[
                    ...
                    <Stat
                        label="Activos"
                        value={empleadosEjemplo.filter(e => e.estado === "activo").length}
                    />
                    <Stat
                        label="Inactivos"
                        value={empleadosEjemplo.filter(e => e.estado === "inactivo").length}
                        muted
                    />
                    <Stat
                        label="Vacaciones"
                        value={empleadosEjemplo.filter(e => e.estado === "vacacion").length}
                        muted
                    />
