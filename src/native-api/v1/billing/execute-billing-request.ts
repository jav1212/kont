...
export async function executeBillingRequest<T>(
  ...
  operation: (actorId: ReturnType<typeof userId>, organization: ReturnType<typeof organizationId>) => Promise<T>,
): Promise<Response> {
  ...
  try {
    ...
    return nativeSuccess(await operation(userId(identity.userId), organizationId(rawOrganizationId)), requestId);
  } catch (cause: unknown) {
