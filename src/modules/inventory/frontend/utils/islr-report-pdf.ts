...
export async function generateIslrReportPdf(
    ...
    for (const p of products) {
        ...
        const openCells: PdfCell[] = [
            ...
            { x: COLS.balQ.x,  w: COLS.balQ.w,  text: formatQty(p.openingQuantity), align: "right", mono: true, size: 7.5, bold: true, color: COLORS.ink },
        ...
        for (const m of p.movements) {
            ...
            const cells: PdfCell[] = [
                ...
                { x: COLS.inQ.x,   w: COLS.inQ.w,   text: m.inboundQuantity > 0 ? formatQty(m.inboundQuantity) : "—",  align: "right", mono: true, size: 7.5, bold: true, color: COLORS.ink },
                { x: COLS.outQ.x,  w: COLS.outQ.w,  text: m.outboundQuantity > 0 ? formatQty(m.outboundQuantity) : "—", align: "right", mono: true, size: 7.5, bold: true, color: COLORS.ink },
                { x: COLS.balQ.x,  w: COLS.balQ.w,  text: formatQty(m.balanceQuantity),           align: "right", mono: true, size: 7.5, bold: true, color: COLORS.ink },
                { x: COLS.inC.x,   w: COLS.inC.w,   text: m.inboundCost > 0 ? formatN(m.inboundCost) : "—",     align: "right", mono: true, size: 7.5, bold: true, color: COLORS.ink },
                { x: COLS.outC.x,  w: COLS.outC.w,  text: m.outboundCost > 0 ? formatN(m.outboundCost) : "—",   align: "right", mono: true, size: 7.5, bold: true, color: COLORS.ink },
