...
export class ExecutePurchaseCsvImportUseCase extends UseCase<{ id: string; companyId: string; mode: PurchaseCsvImportMode; revision: number; targetInvoiceId?: string }, PurchaseCsvImportLineExecution[]> {
    /**
     ...
     */
    async execute(input: { id: string; companyId: string; mode: PurchaseCsvImportMode; revision: number; targetInvoiceId?: string }): Promise<Result<PurchaseCsvImportLineExecution[]>> {
        ...
        return this.repo.execute(input.id, input.companyId, input.mode, input.revision, input.targetInvoiceId);
    }
}
