// Application layer — generates accounting entries when a purchase invoice is confirmed.
// Non-blocking by design: errors are logged and Result.success is returned so
// the inventory operation is never rolled back by an accounting failure.
...
export interface ProcessInventoryPurchaseInput {
    companyId:  string;
    invoiceId:  string;
    date:       string;   // ISO date — invoice date, used to locate open period
    subtotal:   number;
    vatAmount:  number;
    total:      number;
}

export class ProcessInventoryPurchaseIntegrationUseCase {
    constructor(
    ...
    async execute(input: ProcessInventoryPurchaseInput): Promise<Result<void>> {
        // 1. Get active inventory purchase rules for this company
        const rulesResult = await this.ruleRepo.findByCompany(input.companyId, 'inventory_purchase');
        ...
        if (rules.length === 0) {
            await this.log(input, null, 'skipped', 'No hay reglas de integración configuradas para Compras');
            return Result.success(undefined);
        }

        // 2. Find the open accounting period containing the invoice date
        const periodResult = await this.periodRepo.findOpenForDate(input.companyId, input.date);
        ...
        if (!period) {
            await this.log(input, null, 'error', `No hay período contable abierto para la fecha ${input.date}`);
            return Result.success(undefined);
        }
    ...
    private resolveAmount(field: AmountField, input: ProcessInventoryPurchaseInput): number {
        switch (field) {
            case 'subtotal':   return input.subtotal;
            case 'vat_amount': return input.vatAmount;
    ...
    private async log(
        input:   ProcessInventoryPurchaseInput,
        entryId: string | null,
        status:  string,
        error:   string | null,
    ) {
