...
export async function authenticateClientRequest(
  ...
  const identity = await createSupabaseAccessTokenVerifier({
    url,
    anonKey,
  }).verify(accessToken);
  ...
  const client =
    clientHeader === "mobile" || clientHeader === "web"
      ? clientHeader
      : "desktop";
  try {
    await new ObserveNativeSession(
      ...
      operatingSystem: readMetadata(
        request.headers.get("x-kontave-operating-system"),
      ),
    });
  } catch (cause) {
    if (
