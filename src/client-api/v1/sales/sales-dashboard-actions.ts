...
export function createSalesDashboardActions() {
  ...
  if (!url || !key)
    throw new Error("Native sales dashboard infrastructure is not configured.");
  return {
    get: new GetSalesDashboard(
      createSupabaseSalesDashboardReader({ url, serviceRoleKey: key }),
    ),
  };
}
