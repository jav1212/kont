...
export function createProductCommercialActions() {
  ...
  if (!url || !key)
    throw new Error(
      "Native product commercial infrastructure is not configured.",
    );
  const pricing = createSupabaseProductSalePricingRepository({
      url,
      serviceRoleKey: key,
    }),
    taxation = createSupabaseProductTaxationRepository({
      url,
      serviceRoleKey: key,
    }),
  ...
  return {
    getPricing: new GetProductSalePricing(pricing),
