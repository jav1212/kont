...
export async function executeDocumentRequest<T>(
  ...
  try {
    ...
    ).execute(organization, ModuleCapability.DocumentsFiles);
    return apiSuccess(
      await operation(
    ...
    if (
      ...
      cause instanceof AccessControlFailure
    ) {
      return apiError(
        "ORGANIZATION_ACCESS_DENIED",
        "No tienes acceso a Documentos.",
        requestId,
        403,
      );
    }
    ...
    if (cause instanceof DocumentsFailure) {
      console.error("client.documents.operation.failed", {
        requestId,
        code: cause.code,
        organizationId: rawOrganizationId,
        userId: authenticatedUserId,
        client: clientSource(request.headers.get("x-kontave-client")),
      });
      const status = cause.code.includes("VERSION_CONFLICT")
        ? 409
        : cause.code.includes("NOT_FOUND")
          ? 404
          : cause.code === "DOCUMENT_FOLDER_NOT_EMPTY"
            ? 409
            : cause.code.includes("INVALID") || cause.code.includes("OUTSIDE")
              ? 400
              : 503;
      return apiError(cause.code, cause.message, requestId, status);
    }
    console.error("client.documents.failed", {
      requestId,
      organizationId: rawOrganizationId,
      userId: authenticatedUserId,
      client: clientSource(request.headers.get("x-kontave-client")),
