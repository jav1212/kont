...
export function createCompanyActions() {
  ...
  if (!url || !serviceRoleKey)
    throw new Error("Native company infrastructure is not configured.");
  const repository = createCompanyRepository({ url, serviceRoleKey });
  return {
    list: new ListOrganizationCompanies(repository),
    getOperational: new GetOperationalCompany(repository),
  };
}
