...
export function createOrganizationInvitationNotifier(
  origin: string,
  ...
  return {
    async sendInvitation(input) {
      ...
      const html = renderEmailLayout({
        ...
        metaRows: [
          { label: "Organización", value: input.organizationName },
          { label: "Rol", value: input.roleName },
        ],
        badge: "INVITACIÓN",
      });
      ...
      if (result.error)
        throw new Error(`Invitation delivery failed: ${result.error.message}`);
    },
  };
}

function resolveInvitationUrl(
  origin: string,
  target: {
    readonly id: "organization.invitation.accept";
    readonly parameters: { readonly token: string };
  },
): string {
