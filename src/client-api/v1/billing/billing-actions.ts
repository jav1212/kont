...
export function createBillingActions() {
  ...
  if (!url || !serviceRoleKey)
    throw new Error("Native billing infrastructure is not configured.");
  const repository = createOrganizationBillingRepository({
    url,
    serviceRoleKey,
  });
  const authorizationService = createSupabaseAuthorization({
    url,
    serviceRoleKey,
  });
  const authorization: OrganizationBillingAuthorization = {
    async require(input) {
      await authorizationService.require.execute({
  ...
  return {
    ...
    manualPaymentRequests: new ListManualPaymentRequests(
      repository,
      authorization,
    ),
    submitManualPaymentRequest: new SubmitManualPaymentRequest(
      repository,
      authorization,
    ),
    createPaymentReceiptUpload: new CreatePaymentReceiptUpload(
      repository,
      authorization,
      createPaymentReceiptStorage({ url, serviceRoleKey }),
    ),
  };
}
