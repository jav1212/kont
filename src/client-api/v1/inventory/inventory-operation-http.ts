...
import { createInventoryOperationActions } from "./inventory-operation-actions";

export const inventoryRead = permissionCode(PERMISSIONS.INVENTORY_READ),
  inventoryCreate = permissionCode(PERMISSIONS.INVENTORY_CREATE),
  inventoryUpdate = permissionCode(PERMISSIONS.INVENTORY_UPDATE);
type Actions = ReturnType<typeof createInventoryOperationActions>;
export async function executeInventoryOperationRequest<T>(
  request: Request,
  ...
  operation: (
    actions: Actions,
    context: {
      actorUserId: ReturnType<typeof userId>;
      organizationId: ReturnType<typeof organizationId>;
      companyId: ReturnType<typeof companyId>;
    },
  ) => Promise<T>,
): Promise<Response> {
  ...
  try {
    ...
    if (
      ...
      return apiError(
        ...
        "No tienes acceso a las operaciones de inventario.",
        requestId,
        403,
      );
    ...
    if (!url || !key)
      throw new Error(
        "Native inventory operations infrastructure is not configured.",
      );
    ...
    ).execute(organization, ModuleCapability.InventoryMovements);
    return apiSuccess(
      await operation(createInventoryOperationActions(), {
        actorUserId: actor,
        organizationId: organization,
        companyId: company,
      }),
    ...
    if (cause instanceof InventoryFailure) {
      const status =
        cause.code === "INVENTORY_OPERATION_NOT_FOUND"
          ? 404
          : cause.code === "INVENTORY_OPERATION_VERSION_CONFLICT" ||
              cause.code === "INVENTORY_OPERATION_TRANSITION_INVALID" ||
              cause.code === "INVENTORY_PERIOD_CLOSED" ||
              cause.code === "INVENTORY_NEGATIVE_STOCK"
            ? 409
            : cause.code === "INVENTORY_OPERATION_ACCESS_DENIED"
              ? 403
              : cause.code === "INVENTORY_REPOSITORY_UNAVAILABLE"
                ? 503
                : 400;
      return apiError(
        inventoryOperationErrorCode(cause),
        cause.message,
        requestId,
        status,
      );
    ...
    console.error("client.inventory_operations.failed", { requestId, cause });
    return apiError(
      "INTERNAL_ERROR",
      "No se pudo procesar la operación de inventario.",
      requestId,
      500,
    );
  }
...
function inventoryOperationErrorCode(failure: InventoryFailure): ApiErrorCode {
  switch (failure.code) {
    case "INVENTORY_OPERATION_NOT_FOUND":
    case "INVENTORY_OPERATION_VERSION_CONFLICT":
    case "INVENTORY_OPERATION_TRANSITION_INVALID":
    case "INVENTORY_OPERATION_ACCESS_DENIED":
    case "INVENTORY_PERIOD_CLOSED":
    case "INVENTORY_NEGATIVE_STOCK":
    case "INVENTORY_REPOSITORY_UNAVAILABLE":
      return failure.code;
    default:
      return "INVENTORY_OPERATION_INVALID";
  }
}

export function inventoryFlowQuery(
  request: Request,
  ...
  return {
    ...
    to,
    direction,
    reason: (p.get("reason") ?? undefined) as never,
    sourceKind: (p.get("sourceKind") ?? undefined) as never,
