...
export function createReplenishmentActions() {
  ...
  if (!url || !key)
    throw new Error("Native replenishment infrastructure is not configured.");
  return {
    update: new UpdateReplenishmentPolicy(
      createSupabaseReplenishmentPolicyRepository({ url, serviceRoleKey: key }),
    ),
  };
}
