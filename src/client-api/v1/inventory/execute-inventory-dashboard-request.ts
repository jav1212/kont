...
export type InventoryDashboardPart =
  | "dashboard"
  | "summary"
  | "charts"
  | "sales"
  | "purchases";
export async function executeInventoryDashboardRequest(
  request: Request,
  rawOrganizationId: string,
  rawCompanyId: string,
  part: InventoryDashboardPart,
): Promise<Response> {
  ...
  try {
    ...
    const organization = organizationId(rawOrganizationId),
      ...
      permission = permissionCode(PERMISSIONS.INVENTORY_READ);
    const access = (
    ...
    if (
      ...
      return apiError(
        ...
        "No tienes acceso al tablero de inventario.",
        requestId,
        403,
      );
    ...
    if (!url || !key)
      throw new Error(
        "Native inventory dashboard infrastructure is not configured.",
      );
    ...
    const snapshot = await createInventoryDashboardActions().get.execute({
      actorUserId: userId(identity.userId),
      organizationId: organization,
      companyId: organizationCompanyId(rawCompanyId),
      ...query,
    });
    return apiSuccess(selectPart(snapshot, part), requestId);
  } catch (cause) {
    if (cause instanceof InventoryDashboardFailure)
      return apiError(
        ...
        cause.message,
        requestId,
        cause.code === "INVENTORY_DASHBOARD_INVALID"
          ? 400
          : cause.code === "INVENTORY_DASHBOARD_ACCESS_DENIED"
            ? 403
            : 503,
      );
    if (
    ...
    console.error("client.inventory_dashboard.failed", { requestId, cause });
    return apiError(
      "INTERNAL_ERROR",
      "No se pudo obtener el tablero de inventario.",
      requestId,
      500,
    );
  }
...
function readQuery(request: Request) {
  ...
  return {
    ...
    recentLimit: rawLimit === null ? 5 : Number(rawLimit),
  };
}
function selectPart(
  ...
  if (part === "summary") return snapshot.summary;
  if (part === "charts") return snapshot.charts;
  if (part === "sales") return snapshot.recentSales;
  if (part === "purchases") return snapshot.recentPurchases;
  return snapshot;
}
