...
export async function executePurchasingDashboardRequest(
  request: Request,
  ...
  try {
    ...
    if (
      ...
      return apiError(
        ...
        "No tienes acceso al tablero de compras.",
        requestId,
        403,
      );
    ...
    if (!url || !key)
      throw new Error(
        "Native purchasing dashboard infrastructure is not configured.",
      );
    if (
      access.accessPath.kind === OrganizationAccessPathKind.DirectMembership
    ) {
    ...
    ).execute(organization, ModuleCapability.PurchasingDashboard);
    const query = readQuery(request);
    const snapshot = await createPurchasingDashboardActions().get.execute({
    ...
    if (cause instanceof PurchasingDashboardFailure)
      return apiError(
        cause.code,
        cause.message,
        requestId,
        cause.code === "PURCHASING_DASHBOARD_INVALID"
          ? 400
          : cause.code === "PURCHASING_DASHBOARD_ACCESS_DENIED"
            ? 403
            : 503,
      );
    if (
    ...
    console.error("client.purchasing_dashboard.failed", { requestId, cause });
    return apiError(
      "INTERNAL_ERROR",
      "No se pudo obtener el tablero de compras.",
      requestId,
      500,
    );
  }
