...
export function createProfileActions(accessToken: string): {
  ...
  if (!url || !anonKey)
    throw new Error("Native profile access is not configured.");

  const { profiles, avatars } = createSupabaseProfileAdapters({
    url,
    anonKey,
    accessToken,
  });
  return {
    getCurrentProfile: new GetCurrentProfile(profiles),
    updateCurrentProfile: new UpdateCurrentProfile(profiles),
    uploadAvatar: new UploadCurrentProfileAvatar(profiles, avatars),
    deleteAvatar: new DeleteCurrentProfileAvatar(profiles, avatars),
  };
}
