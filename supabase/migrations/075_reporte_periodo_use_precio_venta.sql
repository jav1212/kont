-- =============================================================================
-- 075_reporte_periodo_use_precio_venta.sql
--
-- Actualiza tenant_inventario_reporte_periodo para que la columna
-- `total_salidas_s_iva_bs` refleje el precio de venta cuando esté presente
-- (campo `precio_venta_unitario` introducido en mig 074), con fallback al
-- costo_total para movimientos legacy.
--
-- Adicionalmente:
--   - Agrega total_iva_salidas_bs y total_con_iva_salidas_bs derivados del
--     iva_tipo del producto (general=16% / exento=0%) y del monto sin IVA
--     de cada salida.
--   - Limpia los tipos `entrada_produccion` / `salida_produccion` del CASE
--     porque fueron eliminados con las transformaciones (mig 071).
-- =============================================================================
...
BEGIN
    ...
    EXECUTE format($sql$
        ...
        mov_periodo AS (
            SELECT
                m.producto_id,
                SUM(CASE WHEN m.tipo IN ('entrada','devolucion_salida','ajuste_positivo')
                         THEN m.cantidad ELSE 0 END) AS entradas,
                SUM(CASE WHEN m.tipo IN ('salida','devolucion_entrada','ajuste_negativo','autoconsumo')
                ...
                SUM(CASE WHEN m.tipo = 'salida'
                         THEN COALESCE(m.precio_venta_unitario * m.cantidad, m.costo_total, 0)
                         ELSE 0 END) AS total_salidas_s_iva_bs,
                SUM(CASE WHEN m.tipo IN ('salida','devolucion_entrada','ajuste_negativo','autoconsumo')
        ...
        FROM (
            SELECT
                ...
                ROUND(
                    COALESCE(mp.total_salidas_s_iva_bs, 0)
                    * CASE WHEN p.iva_tipo = 'general' THEN 0.16 ELSE 0 END,
                    2
                ) AS total_iva_salidas_bs,
                ROUND(
                    COALESCE(mp.total_salidas_s_iva_bs, 0)
                    * CASE WHEN p.iva_tipo = 'general' THEN 1.16 ELSE 1 END,
                    2
                ) AS total_con_iva_salidas_bs
