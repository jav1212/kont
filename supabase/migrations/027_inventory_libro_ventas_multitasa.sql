...
-- DT-1: Desglose IVA por alícuota en Libro de Ventas
-- Agrega iva_venta_alicuota a inventario_movimientos y homologa
-- el libro de ventas al patrón del libro de compras (8%/16%/exenta).
-- =============================================================================

-- ---------------------------------------------------------------------------
-- 1. Backfill: agregar iva_venta_alicuota a todos los schemas existentes
-- ---------------------------------------------------------------------------
DO $$
DECLARE
    r        record;
    ...
    FOR r IN SELECT schema_name FROM public.tenants LOOP
        ...
        EXECUTE format(
            $sql$ALTER TABLE %I.inventario_movimientos
                 ADD COLUMN IF NOT EXISTS iva_venta_alicuota text
                     CHECK (iva_venta_alicuota IN ('exenta','reducida_8','general_16'))$sql$,
            v_schema
        );

        -- Backfill salida_venta: derivar alícuota del ratio iva/base
        EXECUTE format($sql$
            UPDATE %I.inventario_movimientos
            SET iva_venta_alicuota = CASE
                WHEN iva_venta_monto IS NULL
                  OR iva_venta_monto = 0
                  OR precio_venta_unitario IS NULL
                  OR precio_venta_unitario * cantidad = 0
                    THEN 'exenta'
                WHEN ABS(iva_venta_monto / (precio_venta_unitario * cantidad) - 0.08) < 0.02
                    THEN 'reducida_8'
                ELSE 'general_16'
            END
            WHERE tipo = 'salida_venta'
              AND iva_venta_alicuota IS NULL
        $sql$, v_schema);
    END LOOP;
END;
$$;
...
-- ---------------------------------------------------------------------------
-- 2. Actualizar provision_tenant_schema — incluye iva_venta_alicuota
--    y factura_compra_id (omitido en 023)
-- ---------------------------------------------------------------------------
...
BEGIN
    ...
    -- inventario_movimientos (con iva_venta_alicuota)
    EXECUTE format($tbl$
...
-- ---------------------------------------------------------------------------
-- 3. tenant_inventario_movimientos_save — persiste iva_venta_alicuota
-- ---------------------------------------------------------------------------
...
BEGIN
    ...
    EXECUTE format($sql$
        ...
        ON CONFLICT (id) DO UPDATE SET
            ...
            iva_venta_alicuota    = EXCLUDED.iva_venta_alicuota
    ...
    $sql$,
        ...
        NULLIF(p_row->>'iva_venta_alicuota', ''),
        v_schema
    ) INTO v_result;
...
-- ---------------------------------------------------------------------------
-- 4. tenant_inventario_movimientos_get — retorna iva_venta_alicuota
-- ---------------------------------------------------------------------------
        ...
        EXECUTE format($q$
            SELECT COALESCE(jsonb_agg(
                jsonb_build_object(
                    ...
                    'iva_venta_alicuota',    m.iva_venta_alicuota,
...
-- ---------------------------------------------------------------------------
-- 5. tenant_inventario_libro_ventas — desglose por alícuota IVA
--    Nuevos campos: base_exenta, base_gravada_8, iva_8, base_gravada_16, iva_16
-- ---------------------------------------------------------------------------
    ...
    EXECUTE format($q$
        ...
        FROM (
            ...
            SELECT
                ...
                ROUND(SUM(CASE WHEN COALESCE(m.iva_venta_alicuota,'general_16') = 'exenta'
                               THEN m.precio_venta_unitario * m.cantidad ELSE 0 END), 2) AS base_exenta,
                ROUND(SUM(CASE WHEN COALESCE(m.iva_venta_alicuota,'general_16') = 'reducida_8'
                               THEN m.precio_venta_unitario * m.cantidad ELSE 0 END), 2) AS base_gravada_8,
                ROUND(SUM(CASE WHEN COALESCE(m.iva_venta_alicuota,'general_16') = 'reducida_8'
                               THEN COALESCE(m.iva_venta_monto, 0) ELSE 0 END), 2)      AS iva_8,
                ROUND(SUM(CASE WHEN COALESCE(m.iva_venta_alicuota,'general_16') = 'general_16'
                               THEN m.precio_venta_unitario * m.cantidad ELSE 0 END), 2) AS base_gravada_16,
                ROUND(SUM(CASE WHEN COALESCE(m.iva_venta_alicuota,'general_16') = 'general_16'
                               THEN COALESCE(m.iva_venta_monto, 0) ELSE 0 END), 2)      AS iva_16,
            ...
            -- Autoconsumos (filas individuales)
            SELECT
                m.id                                                               AS id,
                m.fecha::text                                                      AS fecha,
                ''                                                                 AS numero_factura,
                ...
                0::numeric                                                         AS base_gravada_8,
                0::numeric                                                         AS iva_8,
                0::numeric                                                         AS base_gravada_16,
                0::numeric                                                         AS iva_16,
