-- =============================================================================
-- 104_bonificaciones_history.sql
-- Historial persistente de Bonificaciones por empleado (borradores +
-- confirmados), espejando el patrón de 078_cesta_ticket_history.sql y
-- 102_bono_guerra_history.sql.
--
-- Diferencia clave: cada empleado recibe N bonos (productividad, alimentación,
-- etc.) configurados a nivel de empresa. El recibo por empleado guarda el
-- desglose en jsonb (bonus_lines) más un total VES denormalizado para reporting
-- rápido.
--
-- Agrega:
--   tenant.<schema>.bonificaciones_runs       — un registro por (empresa, periodo)
--   tenant.<schema>.bonificaciones_receipts   — un registro por empleado con
--                                                el desglose de bonos en jsonb
--
-- RPCs públicos:
--   tenant_bonificaciones_run_save(p_user_id, p_run, p_receipts, p_status)
--   tenant_bonificaciones_runs_by_company(p_user_id, p_company_id)
--   tenant_bonificaciones_receipts_by_run(p_user_id, p_run_id)
--
-- La lógica de UPSERT del draft replica 078/102:
...
BEGIN
    FOR r IN SELECT schema_name, id FROM public.tenants LOOP
        ...
        -- bonificaciones_runs
        EXECUTE format($tbl$
            CREATE TABLE IF NOT EXISTS %I.bonificaciones_runs (
        ...
        EXECUTE format(
            'CREATE INDEX IF NOT EXISTS bonificaciones_runs_company_idx ON %I.bonificaciones_runs(company_id)',
            v_schema
        );

        -- bonificaciones_receipts
        EXECUTE format($tbl$
            CREATE TABLE IF NOT EXISTS %I.bonificaciones_receipts (
                id              text          PRIMARY KEY DEFAULT gen_random_uuid()::text,
                run_id          text          NOT NULL REFERENCES %I.bonificaciones_runs(id) ON DELETE CASCADE,
        ...
        EXECUTE format(
            'CREATE INDEX IF NOT EXISTS bonificaciones_receipts_run_idx ON %I.bonificaciones_receipts(run_id)',
            v_schema
        );
        EXECUTE format(
            'CREATE INDEX IF NOT EXISTS bonificaciones_receipts_company_idx ON %I.bonificaciones_receipts(company_id)',
            v_schema
        );

        -- RLS
        EXECUTE format('ALTER TABLE %I.bonificaciones_runs     ENABLE ROW LEVEL SECURITY', v_schema);
        EXECUTE format('ALTER TABLE %I.bonificaciones_receipts ENABLE ROW LEVEL SECURITY', v_schema);

        EXECUTE format('DROP POLICY IF EXISTS tenant_owner ON %I.bonificaciones_runs',     v_schema);
        EXECUTE format('DROP POLICY IF EXISTS tenant_owner ON %I.bonificaciones_receipts', v_schema);
        EXECUTE format(
            'CREATE POLICY tenant_owner ON %I.bonificaciones_runs FOR ALL USING (auth.uid() = %L::uuid)',
            v_schema, r.id
        );
        EXECUTE format(
            'CREATE POLICY tenant_owner ON %I.bonificaciones_receipts FOR ALL USING (auth.uid() = %L::uuid)',
            v_schema, r.id
        );
...
--      el patrón establecido en 078 y 102.
...
BEGIN
    ...
    EXECUTE format('CREATE INDEX IF NOT EXISTS bonificaciones_runs_company_idx ON %I.bonificaciones_runs(company_id)', v_schema);
    ...
    EXECUTE format('CREATE INDEX IF NOT EXISTS bonificaciones_receipts_run_idx     ON %I.bonificaciones_receipts(run_id)',     v_schema);
    EXECUTE format('CREATE INDEX IF NOT EXISTS bonificaciones_receipts_company_idx ON %I.bonificaciones_receipts(company_id)', v_schema);
    ...
    EXECUTE format('CREATE POLICY tenant_owner ON %I.bonificaciones_runs FOR ALL USING (auth.uid() = %L::uuid)',      v_schema, p_user_id);
    EXECUTE format('CREATE POLICY tenant_owner ON %I.bonificaciones_receipts FOR ALL USING (auth.uid() = %L::uuid)',  v_schema, p_user_id);
...
-- ── 3. RPC: tenant_bonificaciones_run_save ──────────────────────────────────
--   UPSERT semantics idénticas a tenant_cesta_ticket_run_save (078) y
--   tenant_bono_guerra_run_save (102), con diferencias menores:
--     * p_run incluye totalVes / employeeCount / lineCount agregados.
--     * cada receipt incluye totalVes + bonusLines (jsonb array).

CREATE OR REPLACE FUNCTION public.tenant_bonificaciones_run_save(
    p_user_id  uuid,
    p_run      jsonb,
    p_receipts jsonb,
...
BEGIN
    IF p_status NOT IN ('draft', 'confirmed') THEN
        RAISE EXCEPTION 'invalid bonificaciones status: %', p_status;
    END IF;

    v_schema := public.tenant_get_schema(p_user_id);

    EXECUTE format(
        'SELECT id, status FROM %I.bonificaciones_runs
    ...
    IF v_existing_id IS NOT NULL THEN
        IF v_existing_status = 'confirmed' THEN
            RAISE EXCEPTION 'A bonificaciones run already exists for this period.';
        END IF;

        v_run_id := v_existing_id;
        EXECUTE format('DELETE FROM %I.bonificaciones_receipts WHERE run_id = %L', v_schema, v_run_id);
        EXECUTE format(
            'UPDATE %I.bonificaciones_runs
             SET status = %L,
                 exchange_rate  = %L::numeric,
                 total_ves      = %L::numeric,
                 employee_count = %L::integer,
                 line_count     = %L::integer,
                 confirmed_at   = now()
             WHERE id = %L',
            v_schema, p_status,
            p_run->>'exchangeRate',
            COALESCE(p_run->>'totalVes', '0'),
            COALESCE(p_run->>'employeeCount', '0'),
            COALESCE(p_run->>'lineCount', '0'),
            v_run_id
        );
    ELSE
        v_run_id := gen_random_uuid()::text;
        EXECUTE format(
            'INSERT INTO %I.bonificaciones_runs
             (id, company_id, period_start, period_end, exchange_rate,
              total_ves, employee_count, line_count, status, confirmed_at)
             VALUES (%L, %L, %L::date, %L::date, %L::numeric,
                     %L::numeric, %L::integer, %L::integer, %L, now())',
            v_schema, v_run_id,
    ...
    FOR v_receipt IN SELECT * FROM jsonb_array_elements(p_receipts) LOOP
        EXECUTE format(
            'INSERT INTO %I.bonificaciones_receipts
             (id, run_id, company_id, employee_id, employee_cedula, employee_nombre, employee_cargo,
              total_ves, bonus_lines)
             VALUES (%L,%L,%L,%L,%L,%L,%L,%L::numeric,%L::jsonb)',
            v_schema,
            ...
            v_receipt->>'totalVes',
            COALESCE(v_receipt->'bonusLines', '[]'::jsonb)::text
        );
    END LOOP;
...
-- ── 4. RPC: tenant_bonificaciones_runs_by_company ────────────────────────────

CREATE OR REPLACE FUNCTION public.tenant_bonificaciones_runs_by_company(
    p_user_id    uuid,
    p_company_id text
)
...
BEGIN
    ...
    EXECUTE format(
        'SELECT COALESCE(jsonb_agg(row_to_json(r) ORDER BY r.confirmed_at DESC), ''[]''::jsonb)
         FROM %I.bonificaciones_runs r WHERE r.company_id = %L',
...
-- ── 5. RPC: tenant_bonificaciones_receipts_by_run ────────────────────────────

CREATE OR REPLACE FUNCTION public.tenant_bonificaciones_receipts_by_run(
    p_user_id uuid,
    p_run_id  text
)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
...
BEGIN
    ...
    EXECUTE format(
        'SELECT COALESCE(jsonb_agg(row_to_json(r) ORDER BY r.employee_nombre), ''[]''::jsonb)
         FROM %I.bonificaciones_receipts r WHERE r.run_id = %L',
        v_schema, p_run_id
    ) INTO v_result;
    RETURN v_result;
END;
...
REVOKE EXECUTE ON FUNCTION public.tenant_bonificaciones_run_save(uuid, jsonb, jsonb, text)
    FROM anon, authenticated, public;

REVOKE EXECUTE ON FUNCTION public.tenant_bonificaciones_runs_by_company(uuid, text)
    FROM anon, authenticated, public;

REVOKE EXECUTE ON FUNCTION public.tenant_bonificaciones_receipts_by_run(uuid, text)
    FROM anon, authenticated, public;
