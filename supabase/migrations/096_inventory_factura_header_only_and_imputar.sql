-- ============================================================================
-- Migration 096: dos flujos de factura de compra
-- ----------------------------------------------------------------------------
-- Flujo completo (existente): proveedor + datos + items detallados → confirmar
-- Flujo rápido (nuevo):       proveedor + total declarado, sin items → confirmar
--                             (asiento contable + libro de compras se generan
--                              al confirmar; inventario queda pendiente).
--                             Items se imputan después desde la bandeja
--                             /inventory/compras-pendientes vía la nueva
--                             RPC tenant_inventario_factura_imputar_items.
--
-- Cambios:
--  1. tenant_inventario_factura_save:
--     - Cuando p_items está vacío, NO recalcula subtotal/iva/total desde items;
--       usa los valores declarados por el cliente en p_factura
--       (subtotal, iva_monto, total). Esto soporta el flujo rápido.
--     - Si vienen items, comportamiento idéntico al actual: subtotal e iva_monto
--       se calculan desde los items (alícuotas mixtas). Los totales del cliente
--       se ignoran.
--
--  3. tenant_inventario_facturas_get: agrega items_count al listado para
--     que el frontend pueda filtrar facturas pendientes de imputar
--     (status='confirmada' AND items_count=0) sin cargar el array completo.
--
--  2. tenant_inventario_factura_imputar_items (NUEVA):
--     - Reemplaza items de una factura ya confirmada que aún no tiene detalle.
--     - Crea los movimientos de inventario (tipo='entrada', factura_compra_id).
--     - Recalcula subtotal/iva/total del header desde los nuevos items.
--     - Si la factura tiene retención IVA configurada, recalcula
--       retencion_iva_monto. NO toca el comprobante (ya asignado al confirmar).
--     - Operación de UN SOLO TIRO: si la factura ya tiene items, falla.
--       Para reimputar hay que desconfirmar/eliminar items y reconfirmar
--       (flujo existente).
-- ============================================================================

-- 1) tenant_inventario_factura_save — header-only support
...
BEGIN
    ...
    IF v_header_only THEN
        -- Flujo rápido: el cliente declara los totales explícitamente.
        v_subtotal  := COALESCE(NULLIF(p_factura->>'subtotal','')::numeric, 0);
        v_iva_monto := COALESCE(NULLIF(p_factura->>'iva_monto','')::numeric, 0);
    ELSE
        -- Flujo completo: subtotal e IVA se calculan desde items con alícuotas.
...
-- 2) tenant_inventario_factura_imputar_items — flujo rápido: imputar items
--    a una factura ya confirmada que se creó sin detalle.
--
-- Reglas:
--  - La factura DEBE estar confirmada y SIN items previos.
--  - Inserta los items, crea movimientos (tipo='entrada'), actualiza producto.
--  - Recalcula subtotal/iva_monto/total del header desde los items.
--  - Si la factura tiene retención IVA, recalcula retencion_iva_monto desde el
--    nuevo iva_monto. El comprobante IVA/ISLR ya asignado NO se toca.
...
BEGIN
    ...
    -- Insertar items + crear movimientos + actualizar productos
    FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
        EXECUTE format($sql$
            INSERT INTO %I.inventario_facturas_compra_items
                (id, factura_id, producto_id, cantidad, costo_unitario, costo_total,
    ...
    -- Recalcular subtotal e IVA del header desde los items recién insertados
    EXECUTE format($sql$
...
-- 3) tenant_inventario_facturas_get — agrega items_count al listado
