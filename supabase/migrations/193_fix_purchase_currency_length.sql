-- Keep purchase-total recalculation compatible with three-letter currency codes.
-- The invoice item currency was migrated from B/D to codes such as VES/USD,
-- but this function still used a one-character local variable.
...
begin
    select
        ...
        case
            when count(i.id) > 0 and bool_and(upper(coalesce(i.currency, 'VES')) in ('USD', 'D'))
                then 'USD'
            else 'VES'
        end,
    ...
    if v_currency = 'USD' and v_rate <= 0 then
    ...
    with raw_lines as (
        select
            ...
            i.vat_rate,
            case
                when v_currency = 'USD' and upper(coalesce(i.currency, 'VES')) in ('USD', 'D')
            ...
            i.discount_type,
            i.discount_value,
            case when v_currency = 'USD' then coalesce(i.discount_amount, 0) / v_rate
                 else coalesce(i.discount_amount, 0) end as discount_amount,
            i.surcharge_type,
            i.surcharge_value,
            case when v_currency = 'USD' then coalesce(i.surcharge_amount, 0) / v_rate
    ...
    header_values as (
        select
            ...
            case
                ...
                when v_currency = 'USD' then coalesce(f.discount_amount, 0) / v_rate
            ...
            case
                ...
                when v_currency = 'USD' then coalesce(f.surcharge_amount, 0) / v_rate
    ...
    v_vat_fiscal := round(v_vat_raw, 2);
    ...
    set subtotal = v_sign * v_subtotal_fiscal,
        ...
        total = v_sign * round(v_subtotal_fiscal + v_vat_fiscal + v_extra_taxes, 2),
        updated_at = now()
