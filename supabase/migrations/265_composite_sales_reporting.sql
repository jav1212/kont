-- Combo revenue belongs to its commercial product, while its component movements
-- carry only inventory cost. Use the confirmed snapshot, never the current recipe.
create or replace function public.shared_inventory_composite_sales_totals(
    p_tenant_id uuid, p_company_id text, p_period text
) returns table(product_id text, sales_value numeric)
language sql
security definer
set search_path = public
as $function$
    select i.product_id, sum(i.unit_price * i.quantity) as sales_value
    ...
    where f.tenant_id = p_tenant_id
      ...
      and exists (
          select 1 from public.shared_inventory_sales_invoice_item_components c
          where c.tenant_id = i.tenant_id and c.invoice_item_id = i.id
      )
    group by i.product_id
$function$;

revoke all on function public.shared_inventory_composite_sales_totals(uuid,text,text) from public,anon,authenticated;
grant execute on function public.shared_inventory_composite_sales_totals(uuid,text,text) to service_role;
...
set search_path = public
as $function$
with composite_sales as (
    select * from public.shared_inventory_composite_sales_totals(p_tenant_id, p_company_id, p_period)
),
period_products as (
    select product_id from composite_sales
    union
    select m.product_id
...
from (
    select
        ...
        coalesce(mp.total_salidas_s_iva_bs, 0) + coalesce(cs.sales_value, 0) as total_salidas_s_iva_bs,
    ...
    left join composite_sales cs on cs.product_id = p.id
