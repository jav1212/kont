-- ---------------------------------------------------------------------------
-- 024 — RPC: Reporte SALDO por Departamento
-- ---------------------------------------------------------------------------
-- Agrupa el inventario del período por departamento, calculando:
-- unidades y costos de: inventario inicial, entradas, salidas, existencia actual.
-- ---------------------------------------------------------------------------
...
BEGIN
    ...
    EXECUTE format($sql$
        WITH
        -- Último saldo por producto antes del período (inventario inicial)
        inv_inicial AS (
            SELECT
                m.producto_id,
        ...
        -- Movimientos del período agrupados por producto
        mov_periodo AS (
            SELECT
                m.producto_id,
        ...
        -- Datos por producto con su departamento
        por_producto AS (
            SELECT
