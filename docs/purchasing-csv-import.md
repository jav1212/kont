...
`Sub Total Bs.` es el importe fuente autoritativo en bolívares. Se conserva incluso cuando `Cantidad × Costo Bs.` no coincide por el redondeo del costo unitario que exportó el sistema de origen. Al preparar el detalle, esa base se limita a cuatro decimales —la escala de persistencia— antes de agregar los importes fiscales. Al recalcular una factura de este formato, se usa la base de IVA persistida y no se vuelve a derivar desde el costo unitario redondeado. Así, los totales guardados conservan la previsualización en casos de precisión como las facturas `1771281` y `1771283`. `Costo Bs.` se usa para calcular el costo unitario almacenado; las cifras de otra moneda, incluido `S. Total Otra Moneda`, son solo referencia y no reconstruyen los bolívares ni el total fiscal.

Los costos unitarios y las bases de IVA se almacenan con hasta cuatro decimales, el subtotal fiscal se redondea a céntimos y el IVA agregado se trunca a céntimos. Las tasas admiten hasta cuatro decimales. En compras completas en VES, la tasa del reporte se guarda como metadato de la cabecera; no vuelve a convertir los importes ya expresados en bolívares. `Costo Full Bs.` se muestra como referencia y no altera existencia, precios ni IVA de venta.
...
Además de las migraciones previas [248_shared_purchase_csv_imports.sql](../supabase/migrations/248_shared_purchase_csv_imports.sql) y [250_preserve_purchase_import_line_tenant_on_invoice_delete.sql](../supabase/migrations/250_preserve_purchase_import_line_tenant_on_invoice_delete.sql), el reporte completo requiere [253_purchase_csv_import_draft_resume.sql](../supabase/migrations/253_purchase_csv_import_draft_resume.sql), [258_complete_purchase_csv_import.sql](../supabase/migrations/258_complete_purchase_csv_import.sql) y [259_purchase_csv_authoritative_totals.sql](../supabase/migrations/259_purchase_csv_authoritative_totals.sql). La 258 hace que el ejecutor resuelva el proveedor del reporte completo por nombre, preserve el control de borradores existentes y trate las confirmadas como inmutables e idempotentes. La 259 conserva la base fiscal autoritativa del CSV y recupera la tasa de la cabecera bloqueada cuando un cliente Web anterior no la envía.

El 14 de septiembre de 2026 se aplicaron en Supabase remoto las migraciones 253 (`20260914144744`), 258 (`20260914144800`), 259 (`20260914150541`) y [260_restore_complete_purchase_csv_rates.sql](../supabase/migrations/260_restore_complete_purchase_csv_rates.sql) (`20260914150555`). La 260 completó la tasa de 14 facturas importadas que cumplían sus condiciones de seguridad: VES, tasa faltante y una única cabecera completa enlazada por tenant, empresa, fecha y documento. Las 14 conservaron sus importes y estado; también se verificó que sus 51 renglones y 51 movimientos de inventario permanecieran sin cambios. La 260 no modifica tasas existentes, importes financieros, estado ni inventario. El historial remoto y el hash de la función de la 258 coinciden con la migración local. Las RPC de guardado y ejecución dirigida o reanudable se verificaron como `SECURITY DEFINER`, con `search_path` en `public` y ejecución exclusiva de `service_role`; no se importaron compras durante el despliegue de las migraciones. No hay rollback de datos automático: antes de retirar la capacidad conserve los lotes y facturas generados y prepare una migración compatible de reversión.

Las 32 pruebas focalizadas pasaron, incluido el reporte de muestra `compras completas.csv`: 16 compras y 58 renglones sin errores de análisis. `pnpm build` terminó correctamente y la prueba de renderizado SSR verificó los estados de importación. La comprobación de lint focalizada pasó; el `pnpm lint` completo terminó con 18 errores y 339 advertencias en archivos ajenos a esta capacidad.

El ejecutor SQL de regresión [test-purchase-complete-csv-sql.mjs](../scripts/test-purchase-complete-csv-sql.mjs) pasó con `node --import tsx scripts/test-purchase-complete-csv-sql.mjs`; `PURCHASE_CSV_SAMPLE_PATH` permite indicar el CSV completo de muestra. Carga el recálculo fiscal de producción, las funciones base de importación de la migración 248 y los envoltorios de las migraciones 258 y 259. Verifica las 16 compras y 58 renglones del reporte tanto con costos que incluyen IVA como sin incluirlo, la base autoritativa con la precisión numérica de producción, descuentos, protecciones de retrorelleno de tasa y la preservación de los flujos estándar y `source_bs`; también cubre el reemplazo de un borrador existente al conservar identificador y control, la omisión inmutable de una confirmada y las protecciones de tenant, proveedor por nombre y factura ambigua.
