...
- `src/modules/tools/frontend/status/components/` — portal status module (StatusShell, ServiceDetail, etc.).
- `tools-shell.tsx` is the BCV orchestrator; mounted in `(marketing)/herramientas/divisas` (public+SEO) and `(app)/tools/divisas` (authed).
- `status-shell.tsx` is the status orchestrator; mounted in `(marketing)/herramientas/status` and `(app)/tools/status`. Same `variant: "public" | "authed"` pattern.
- Data layer is via `useBcvRates`, `useBcvHistory`, and `useStatusServices` hooks with `fetch("/api/...")`. Do not change data layer.
- Shared within the BCV module: `CurrencyInlineSelect` (sizes sm/md/lg, wraps HeroUI Select with Flag SVG + code), `AnimatedNumber` (AnimatePresence crossfade on value change), `RateSparkline` (tiny SVG trend line consuming `useBcvHistory`), and `RateCard.Skeleton` (compound loading state). Prefer reusing these over duplicating the patterns.
- Shared within the status module: `UptimeSparkline` (SVG uptime trend from `ServiceWithStatus[]`), `StatusTabs` (public + compact authed variants, URL-driven filter chips), `UptimeBars` (90d day-by-day bars with HeroUI Tooltip per cell), `StatusBadge` (dot + label), `ServiceRow`, `CategorySection`.
- Stagger convention for the shell: `STAGGER_STEP = 0.08s` between sections, `duration 0.35`, `ease "easeOut"`, `y: 8 → 0`, on mount only (not on re-renders). `const section = (i: number) => (...)` helper is duplicated inline per shell file — do not extract into a shared primitive; keeping it local matches the BCV pattern.
...
- Status hero operational count: `text-[40px]` + `/ total` at `text-[20px]`
- Status detail service name: `text-[28px] sm:text-[32px]`
- Status uptime % cards: `text-[24px]`
...
## HeroUI Tooltip conventions (tools module)
- Always use `delay={200}` so hover doesn't trigger on accidental passes.
- Override `classNames={{ base: "bg-transparent", content: "bg-background border border-border-light rounded-lg px-3 py-2 text-foreground" }}` to get the flat KONT look instead of HeroUI's default dark popover with shadow. The library-internal shadow on the popover wrapper remains (acknowledged exception in flat-UI rule).
- Inside the tooltip content, use `font-mono` + `text-[11px]` uppercase labels on top and `text-[12px]` body, same cadence as the rest of the tools module.
