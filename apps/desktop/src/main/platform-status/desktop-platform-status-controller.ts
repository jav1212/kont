...
export class DesktopPlatformStatusController {
  ...
  constructor(
    private readonly source: DesktopPlatformStatusSource,
    private readonly getWindow: () => BrowserWindow | undefined,
  ) {}
  getState(): DesktopPlatformStatusState {
    return this.state;
  }
  ...
  clear(): DesktopPlatformStatusState {
    return this.update({ status: "unavailable" });
  }
  private update(
    state: DesktopPlatformStatusState,
  ): DesktopPlatformStatusState {
    this.state = state;
    this.getWindow()?.webContents.send(
      DESKTOP_IPC.platformStatusChanged,
      state,
    );
    return state;
  }
