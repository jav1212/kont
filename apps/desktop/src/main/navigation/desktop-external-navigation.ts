...
import type {
  ...
  DesktopExternalNavigationResult,
...
export class DesktopExternalNavigation {
  ...
  async open(input: unknown): Promise<DesktopExternalNavigationResult> {
    if (!isDestination(input))
      return {
        ok: false,
        error: { message: "El destino solicitado no es válido." },
      };
    ...
    if (
      ...
      return {
        ok: false,
        error: { message: "El destino solicitado no es seguro." },
      };
    }
    try {
      ...
      return {
        ok: false,
        error: { message: "No se pudo abrir el destino en el navegador." },
      };
    }
  }
}
