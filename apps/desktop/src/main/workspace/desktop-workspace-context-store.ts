...
export class DesktopWorkspaceContextStore implements WorkspaceContextStore {
  private readonly filePath = join(
    ...
    "workspace-context-v2.json",
  );

  async read(): Promise<PersistedWorkspaceContext> {
    try {
      const value = parseContext(
  ...
  async write(context: PersistedWorkspaceContext): Promise<void> {
    ...
    await writeFile(temporaryPath, JSON.stringify(context), {
      encoding: "utf8",
      mode: 0o600,
    });
    try {
  ...
  private async readLegacyContext(): Promise<PersistedWorkspaceContext> {
    ...
    if (!organization)
      return { organizationId: null, companyId: null, moduleCode: null };
...
function readOrganizationId(value: unknown): OrganizationId | null {
  return typeof value === "string" && value.trim()
    ? organizationId(value)
    : null;
}
function readCompanyId(value: unknown): CompanyId | null {
  return typeof value === "string" && value.trim() ? companyId(value) : null;
}
