...
export class DesktopWorkspaceSelectionStore
  implements ActiveWorkspaceSelectionStore
{
  ...
  async read(): Promise<OrganizationId | null> {
    try {
      ...
      if (
        ...
        !("activeWorkspaceId" in parsed)
      )
        return null;
      const value = (parsed as { readonly activeWorkspaceId?: unknown })
        .activeWorkspaceId;
  ...
  async write(activeWorkspaceId: OrganizationId | null): Promise<void> {
    ...
    await writeFile(temporaryPath, JSON.stringify({ activeWorkspaceId }), {
      encoding: "utf8",
      mode: 0o600,
    });
    try {
