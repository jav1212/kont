...
/** Group of related entries in the Desktop settings index. */
...
/** Inputs required to render the Desktop settings index. */
...
/**
 * Renders the grouped settings index.
 * @param props - Settings sections and destination selection callback.
 * @returns Desktop settings index.
 */
    ...
    ? sections
        .map((section) => ({
          ...
          entries: section.entries.filter((entry) =>
            normalizeSearch(`${entry.label} ${entry.description}`).includes(
              normalizedQuery,
            ),
          ),
        }))
  ...
  return (
    ...
    >
      <header className="desktop-settings__heading">
        ...
        <p>
          Administra tu cuenta, organización, empresa y preferencias de Kontave.
        </p>
      </header>
      ...
      {visibleSections.length > 0 ? (
        <div className="desktop-settings__sections">
          {visibleSections.map((section) => (
            <section className="desktop-settings-section" key={section.id}>
              <h3>{section.label}</h3>
              <div className="desktop-settings-list">
                {section.entries.map((entry) => (
                  ...
                  >
                    <span
                      className="desktop-settings-row__icon"
                      aria-hidden="true"
                    >
                      {settingsIcon(entry.iconKey)}
                    </span>
                    ...
                    {entry.availability === "read_only" ? (
                      <span className="desktop-settings-row__mode">
                        Sólo lectura
                      </span>
                    ) : null}
                    <ArrowRight
                      className="desktop-settings-row__arrow"
                      aria-hidden="true"
                    />
                  </button>
                ))}
        ...
        <p className="desktop-settings__empty">
          No encontramos configuraciones para “{query.trim()}”.
        </p>
      )}
    </section>
  );
