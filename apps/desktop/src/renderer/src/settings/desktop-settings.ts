...
const resolver = new ResolveAvailableSettings(
  new StaticSettingsCatalog(SETTINGS_SECTIONS, SETTINGS_ENTRIES),
);
...
const messages: Readonly<Partial<Record<SettingsMessageKey, string>>> = {
  ...
  "settings.account_profile.description":
    "Identidad, avatar y datos personales.",
  ...
  "settings.account_security.description":
    "Contraseña y seguridad de la cuenta.",
  "settings.organization_general.label": "Información de la organización",
  "settings.organization_general.description":
    "Nombre y datos generales de la organización.",
  "settings.organization_members.label": "Miembros",
  "settings.organization_members.description":
    "Personas con acceso a la organización.",
  "settings.organization_roles.label": "Roles y permisos",
  "settings.organization_roles.description":
    "Permisos disponibles para cada tipo de usuario.",
  ...
  "settings.application_devices.description":
...
export function resolveDesktopSettings(input: {
  ...
  const activeWorkspace =
    workspace?.workspaces.find(
      (entry) => entry.id === workspace.activeWorkspaceId,
    ) ?? null;
  const resolved = resolver.execute({
    platform: Platform.Desktop,
    connectivity:
      input.connectivity.availability === "unavailable" ||
      (input.connectivity.availability === "unknown" &&
        input.connectivity.reason !== null)
        ? "offline"
        : "online",
