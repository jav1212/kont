...
/**
 * Selects the appropriate authentication screen for the current session state.
 * @param props - Session state and successful-authentication callback.
 * @returns Authentication experience for anonymous or loading users.
 */
        ...
        <aside className="auth-visual" aria-label="Kontave">
          ...
          <div className="auth-visual__content">
            ...
            <div className="auth-visual__message">
              <Text className="auth-visual__eyebrow" tone="inherit">
                Gestión empresarial venezolana
              </Text>
              <h1>
                Todo tu negocio,
                <br />
              ...
              <p>
                Nómina, inventario y operaciones conectadas en una experiencia
                diseñada para trabajar con claridad.
              </p>
            </div>
            <footer className="auth-visual__footer">
              <Text tone="inherit">Preparado para tu operación</Text>
              <div
                className="auth-visual__capabilities"
                aria-label="Capacidades de Kontave"
              >
                <Text as="strong" tone="inherit">
                  LOTTT
                </Text>
                <Text as="strong" tone="inherit">
                  SENIAT
        ...
        <div className="auth-form-panel">
          <div className="auth-form-panel__content">
            {state.status === "loading" ? (
              <Card className="auth-card auth-loading">
                Restaurando sesión segura…
              </Card>
            ) : null}
            {state.status === "anonymous" && screen === "sign-in" ? (
              <SignInForm
            ...
            {state.status === "anonymous" && screen === "registration" ? (
              <RegistrationForm
                onAuthenticated={onAuthenticated}
                onBack={() => setScreen("sign-in")}
              />
            ) : null}
            {state.status === "anonymous" && screen === "recovery" ? (
              <PasswordRecoveryForm onBack={() => setScreen("sign-in")} />
            ) : null}
          </div>
        </div>
      </section>
