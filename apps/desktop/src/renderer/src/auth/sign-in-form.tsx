...
/**
 * Authenticates an existing Desktop user with email and password.
 * @param props - Authentication and alternate-flow callbacks.
...
export function SignInForm({
    ...
    try {
      ...
      presentFeedback.execute(
        errorFeedback("No se pudo comunicar con Kontave.", {
          deduplicationKey: "auth-sign-in-communication",
        }),
      );
    } finally {
      setLoading(false);
  ...
  return (
    <Card className="auth-card" aria-labelledby="sign-in-title">
      <AuthHeading
        label={<LogoFull size={20} />}
        title="Bienvenido de nuevo"
        description="Ingresa a tu cuenta para continuar con tu operación."
...
/**
 * Renders the shared heading used by authentication screens.
 * @param props - Eyebrow label, title, description, and title identifier.
 * @returns Authentication heading block.
 */
export function AuthHeading({
  ...
  return (
    <div className="auth-heading">
      {typeof label === "string" ? (
        <Text className="desktop-label" tone="subtle">
          {label}
        </Text>
      ) : (
