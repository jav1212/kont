...
/**
 * Guides the user through the multi-step password recovery flow.
 * @param props - Callback used to return to sign-in.
 * @returns Password recovery form.
 */
export function PasswordRecoveryForm({
  ...
  async function request(event: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    await perform(async () => {
      const result = await window.kontave.auth.requestPasswordRecovery({
        email,
      });
      if (result.ok) {
  ...
  async function verify(event: FormEvent<HTMLFormElement>): Promise<void> {
    event.preventDefault();
    await perform(async () => {
      const result = await window.kontave.auth.verifyPasswordRecovery({
        email,
        code,
      });
      if (result.ok) {
  ...
  async function complete(event: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    await perform(async () => {
      const result = await window.kontave.auth.completePasswordRecovery({
        password,
      });
      if (result.ok) {
        setPassword("");
    ...
    try {
      ...
      presentFeedback.execute(
        errorFeedback("No se pudo comunicar con Kontave.", {
          deduplicationKey: "auth-recovery-communication",
        }),
      );
    } finally {
      setLoading(false);
  ...
  return (
    <Card className="auth-card" aria-labelledby="recovery-title">
      {stage === "email" ? (
        <>
          <AuthHeading
            label="Recuperación"
            title="Recuperar acceso"
            description="Te enviaremos un código de 8 dígitos para verificar tu identidad."
      ...
      {stage === "verification" ? (
        <>
          <AuthHeading
            ...
            description={`Ingresa el código enviado a ${email}.`}
            titleId="recovery-title"
          />
      ...
      {stage === "password" ? (
        <>
          <AuthHeading
            ...
            description="La nueva contraseña cerrará la sesión temporal de recuperación."
            titleId="recovery-title"
          />
          <form
            className="auth-form"
