...
/**
 * Renders the inventory overview for the active operational context.
...
export function InventoryDashboardView({ companyId, organizationId }: InventoryDashboardViewProps) {
  ...
  const exchangeRate =
    snapshot.exchangeRates.rates.find(
      ({ baseCurrency }) => baseCurrency === displayCurrency,
    ) ?? null;
  const currencyOptions = [
    {
      value: "VES",
      label: "Bolívar venezolano",
      description: "Moneda base del inventario",
  ...
  const formatDashboardAmount = (value: string): string =>
    formatAmount(value, displayCurrency, exchangeRate);
    ...
    <section className="inventory-dashboard">
      <div className="inventory-dashboard__heading">
        <div>
          ...
          <p>Actividad y valoración de la empresa seleccionada.</p>
        </div>
      ...
      <div className="inventory-dashboard__metrics">
        <Metric
          ...
          value={formatDashboardAmount(summary.inboundValue.amount)}
        />
        <Metric
          icon={<ArrowUpFromLine />}
          tone="violet"
          label="Valor de salidas"
          value={formatDashboardAmount(summary.outboundValue.amount)}
        />
        <Metric
          icon={<Repeat2 />}
          tone="pink"
          label="Movimientos"
          value={summary.movementCount.toLocaleString("es-VE")}
        />
        <Metric
          icon={<Boxes />}
          tone="cyan"
          label="Valor del inventario"
          value={formatDashboardAmount(summary.inventoryValue.amount)}
        />
      </div>

      <div className="inventory-dashboard__charts">
        <ChartCard title="Entradas vs. salidas">
          <FlowChart points={dashboard.charts} />
        </ChartCard>
        <ChartCard title="Valor de entradas">
          <BarChart points={dashboard.charts} field="inboundValue" />
        </ChartCard>
        <ChartCard title="Cantidad de movimientos">
          <BarChart points={dashboard.charts} field="movementCount" />
        </ChartCard>
      </div>
      ...
      <div className="inventory-dashboard__tables">
        <MovementTable
          ...
          empty="No hay salidas en este período."
          movements={dashboard.recentOutboundMovements}
          formatValue={formatDashboardAmount}
        />
        <MovementTable
          title="Últimas entradas"
          empty="No hay entradas en este período."
          movements={dashboard.recentInboundMovements}
          formatValue={formatDashboardAmount}
        />
      </div>
    </section>
...
function Metric({
  ...
  return (
    <article className="inventory-metric">
      <span
        className={`inventory-metric__icon inventory-metric__icon--${tone}`}
      >
        {icon}
      </span>
      <div>
...
function FlowChart({
  ...
  readonly points: readonly InventoryDashboardChartPointDto[];
}) {
  const width = 320;
  const height = 128;
  const values = points.flatMap((point) => [
    Number(point.inboundValue.amount),
    Number(point.outboundValue.amount),
  ]);
  const max = Math.max(1, ...values);
  const path = (field: "inboundValue" | "outboundValue") =>
    points
      .map((point, index) => {
        const x =
          points.length <= 1
        ...
        const y =
          height - (Number(point[field].amount) / max) * (height - 18) - 8;
        return `${x},${y}`;
      })
      .join(" ");
  ...
  return (
    <div className="inventory-chart">
      <div className="inventory-chart__legend">
        <span className="is-inbound">Entradas</span>
        <span className="is-outbound">Salidas</span>
      </div>
      <svg
        viewBox={`0 0 ${width} ${height}`}
        role="img"
        aria-label="Entradas y salidas del período"
      >
        <polyline className="is-inbound" points={path("inboundValue")} />
        <polyline className="is-outbound" points={path("outboundValue")} />
      </svg>
...
function BarChart({
  ...
  readonly field: "inboundValue" | "movementCount";
  readonly points: readonly InventoryDashboardChartPointDto[];
}) {
  ...
  if (!points.length) return <ChartEmpty />;
  return (
    <div className="inventory-chart inventory-chart--bars">
...
function ChartEmpty() {
  return (
    <div className="inventory-chart__empty">Sin actividad en el período</div>
  );
}
...
function MovementTable({
  ...
  readonly movements: readonly RecentInventoryMovementDto[];
  ...
  return (
    <article className="inventory-documents">
      ...
      {movements.length ? (
        <div className="inventory-documents__list">
          {movements.map((movement) => (
            <div key={movement.id}>
              ...
              <div>
                <strong>{formatValue(movement.totalCost.amount)}</strong>
                <span>
                  {formatDecimalQuantity(movement.quantity.value)}{" "}
                  {unitLabel(movement.quantity.unit)} ·{" "}
                  {formatShortDate(movement.effectiveDate)}
                </span>
              </div>
            </div>
          ))}
        ...
        <p className="inventory-documents__empty">{empty}</p>
      )}
    </article>
  );
}
  ...
  return (
    <section className="inventory-dashboard" aria-busy="true">
      <div className="inventory-dashboard__heading">
        ...
        <Skeleton variant="control" width={210} height={40} />
      </div>
      <div className="inventory-dashboard__metrics">
      ...
      <div className="inventory-dashboard__charts">
        {Array.from({ length: 3 }, (_, index) => (
          <Skeleton key={index} variant="rectangle" width="100%" height={250} />
        ))}
      </div>
      <div className="inventory-dashboard__tables">
...
function formatAmount(
  value: string,
  currency: string,
  rate: { readonly value: string } | null,
): string {
  const numericRate = rate ? Number(rate.value) : null;
  const amount =
    currency !== "VES" && numericRate && numericRate > 0
      ? Number(value) / numericRate
      : Number(value);
    ...
    (
      {
        ...
        devolucion_salida: "Devolución de salida",
        devolucion_venta: "Devolución",
        salida: "Salida",
        salida_venta: "Salida",
        salida_produccion: "Salida de producción",
        ajuste_negativo: "Ajuste negativo",
        devolucion_entrada: "Devolución de entrada",
...
function unitLabel(
  value: RecentInventoryMovementDto["quantity"]["unit"],
): string {
  return {
    each: "unid.",
    kilogram: "kg",
...
function monthQuery(
  ...
  return {
    from,
    to:
      effectiveDate.startsWith(month) && effectiveDate < monthEnd
        ? effectiveDate
        : monthEnd,
  };
}
