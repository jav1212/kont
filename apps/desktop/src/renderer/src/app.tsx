...
import { DesktopSettingsView } from "./settings/settings-view";
import {
  DesktopSettingsDetailView,
  type DesktopSettingsDestination,
} from "./settings/settings-detail-view";
import { InventoryDashboardView } from "./inventory/inventory-dashboard-view";
import { SalesDashboardView } from "./sales/sales-dashboard-view";
import { PurchasingDashboardView } from "./purchasing/purchasing-dashboard-view";
import { ProductsView } from "./products/products-view";
import { ProductCategoriesView } from "./products/product-categories-view";
import { ProductDetailPage } from "./products/product-detail-page";
import { InventoryOperationsView } from "./inventory/inventory-operations-view";
import { resolveDesktopSettings } from "./settings/desktop-settings";
import {
...
/**
 * Composes the authenticated Desktop shell and its module routing.
 * @returns Root Desktop application view.
 */
export function App() {
  const [auth, setAuth] = useState<DesktopAuthState>({ status: "loading" });
  const [workspace, setWorkspace] = useState<DesktopWorkspaceState>({
    status: "loading",
  });
  const [currentUser, setCurrentUser] = useState<DesktopCurrentUserState>({
    status: "loading",
  });
...
function DesktopAppShell({
  ...
  const sidebarPresentation = compactViewport
    ? "drawer"
    : sidebarPinned
      ? "persistent"
      : "collapsed";
  const closeDrawer = (): void => {
    if (compactViewport) setDrawerOpen(false);
  };
    ...
    void window.kontave.navigation.openExternal(destination).then((result) => {
      ...
      presentFeedback.execute(
        errorFeedback(result.error.message, {
          deduplicationKey: `external-navigation-${destination}`,
        }),
      );
    });
  }
  ...
  const breadcrumbs = desktopBreadcrumbs(
    activeNavigationTarget,
    productDetailTitle
      ? { "inventory.product-detail": productDetailTitle }
      : {},
  );
  const settingsSections = useMemo(
    () => resolveDesktopSettings({ auth, connectivity, workspace }),
    [auth, connectivity, workspace],
  );
  const settingsActive =
    activeNavigationTarget?.id === "settings" ||
    activeNavigationTarget?.id.startsWith("settings.") === true;
  const settingsDetail = isDesktopSettingsDestination(
    activeNavigationTarget?.id,
  )
    ? activeNavigationTarget.id
    : null;
  const inventoryDashboardActive =
    activeNavigationTarget?.id === "inventory.dashboard";
  const salesDashboardActive = activeNavigationTarget?.id === "sales.dashboard";
  const purchasingDashboardActive =
    activeNavigationTarget?.id === "purchases.dashboard";
  const inventoryProductsActive =
    activeNavigationTarget?.id === "inventory.products";
  const inventoryProductDetailId =
    activeNavigationTarget?.id === "inventory.product-detail"
      ? activeNavigationTarget.parameters.productId
      : null;
  const inventoryProductCategoriesActive =
    activeNavigationTarget?.id === "inventory.product-categories" ||
    activeNavigationTarget?.id === "inventory.departments";
  const inventoryFlowMode =
    activeNavigationTarget?.id === "inventory.inputs"
      ? "inputs"
      : activeNavigationTarget?.id === "inventory.outputs"
        ? "outputs"
        : activeNavigationTarget?.id === "inventory.operations"
          ? "operations"
          : null;
  ...
  return (
    <div className="desktop-shell">
      ...
      <WorkspaceSidebar
        ...
        closeIcon={<X />}
        headerActionIcon={
          sidebarPinned ? <PanelLeftClose /> : <PanelLeftOpen />
        }
        headerActionLabel={
          sidebarPinned ? "Contraer barra lateral" : "Expandir barra lateral"
        }
        ...
        activeCompanyId={
          workspace.status === "ready" ? workspace.activeCompanyId : null
        }
        activeWorkspaceId={
          workspace.status === "ready" ? workspace.activeWorkspaceId : null
        }
        workspaces={
          workspace.status === "ready" && workspace.workspaces.length > 1
        ...
        account={{
          ...
          ...(personalIdentity?.avatarUrl
            ? { avatarUrl: personalIdentity.avatarUrl }
            : {}),
          ...(billingPlan.status === "ready" && billingPlan.planName
      ...
      <div className="desktop-workspace">
        ...
        <main
          className={`desktop-content${settingsActive ? " desktop-content--settings" : ""}${inventoryDashboardActive || purchasingDashboardActive || salesDashboardActive ? " desktop-content--inventory-dashboard" : ""}${inventoryProductsActive || inventoryProductCategoriesActive || inventoryProductDetailId || inventoryFlowMode ? " desktop-content--products" : ""}`}
        >
          {connectivity.availability === "degraded" ? (
            <Alert intent="warning" className="desktop-connectivity-notice">
          ...
          {settingsDetail ? (
            ...
            <ProductsView
              ...
              onOpenProduct={(productId) =>
                setActiveNavigationTarget(
                  dynamicNavigationTarget("inventory.product-detail", {
                    productId,
                  }),
                )
              }
            />
          ) : inventoryProductCategoriesActive &&
            workspace.status === "ready" &&
            workspace.activeWorkspaceId &&
            workspace.activeCompanyId ? (
            ...
            <ProductDetailPage
              ...
              onTitleChange={setProductDetailTitle}
            />
          ) : inventoryFlowMode &&
            workspace.status === "ready" &&
            workspace.activeWorkspaceId &&
...
function UpdateNotice() {
  ...
  if (state.status === "downloading") {
    ...
    return (
      <Alert intent="warning">
        Descargando la actualización{progress}. Puedes continuar trabajando.
      </Alert>
    );
  }
...
function retryUpdate(
  state: Extract<ClientUpdateSnapshot, { status: "failed" }>,
): Promise<ClientUpdateSnapshot> {
  if (state.failure.operation === "download")
    return window.kontave.updates.download();
  if (state.failure.operation === "apply")
    return window.kontave.updates.apply();
  return window.kontave.updates.check();
}

function useCompactDesktopViewport(): boolean {
  const [compact, setCompact] = useState(
    () => window.matchMedia(DESKTOP_COMPACT_QUERY).matches,
  );
...
function DeviceCard() {
  ...
  useEffect(() => {
    ...
    return window.kontave.devices.subscribe((event) => {
      if (event.type === "device.state-changed")
        setStatus((current) => ({ ...current, state: event.state }));
      if (event.type === "device.connected")
        setStatus({ state: "ready", device: event.device });
  ...
  async function connect(): Promise<void> {
    ...
    try {
      ...
      setError(readErrorMessage(cause, "No se pudo conectar el dispositivo."));
    } finally {
      setConnecting(false);
    }
  }
  ...
  return (
    <Card className="device-card" aria-labelledby="device-title">
      <div className="device-heading">
        <div>
          <Text className="desktop-label">Conexión local</Text>
          <h2 id="device-title">Scanner de códigos</h2>
        </div>
        <StatusBadge intent={statusIntent(status.state)}>
          {status.state}
        </StatusBadge>
      </div>
      <dl className="device-details">
        ...
        <dd>
          {status.device
            ? `${status.device.manufacturer} ${status.device.model}`
            : "Sin dispositivo"}
        </dd>
        <dt>Conexión</dt>
        <dd>{status.device?.connectionAddress ?? "—"}</dd>
      </dl>
...
function accountDisplayName(
  auth: Extract<DesktopAuthState, { status: "authenticated" }>,
): string {
...
function toSidebarWorkspace(entry: DesktopWorkspaceEntry) {
  ...
  const relationship =
    entry.relationship === "personal" ||
    entry.relationship === "member" ||
    entry.relationship === "delegated"
      ? entry.relationship
      : entry.access === "delegated"
        ? "delegated"
        : "member";
  return {
    id: entry.id,
...
function isExternalDestination(
  value: string,
): value is DesktopExternalDestination {
  return (
    value === "settings" ||
    value === "profile" ||
    value === "help" ||
...
function isDesktopSettingsDestination(
  value: string | undefined,
): value is DesktopSettingsDestination {
  return (
...
function statusIntent(
  state: DesktopDeviceStatus["state"],
